...
<span data-ttu-id="987d0-105">CSS yalıtımı, genel stillerdeki bağımlılıkları önleyerek uygulamanın CSS parmak izini basitleştirir ve bileşenler ve kitaplıklar arasında stil çakışmalarını önlemeye yardımcı olur.</span><span class="sxs-lookup"><span data-stu-id="987d0-105">CSS isolation simplifies an app's CSS footprint by preventing dependencies on global styles and helps to avoid styling conflicts among components and libraries.</span></span>

## <a name="enable-css-isolation"></a><span data-ttu-id="987d0-106">CSS yalıtımını etkinleştir</span><span class="sxs-lookup"><span data-stu-id="987d0-106">Enable CSS isolation</span></span> 

<span data-ttu-id="987d0-107">Bileşene özgü stiller tanımlamak için, `.razor.css` bileşenin dosya adıyla eşleşen bir dosya oluşturun `.razor` .</span><span class="sxs-lookup"><span data-stu-id="987d0-107">To define component-specific styles, create a `.razor.css` file matching the name of the `.razor` file for the component.</span></span> <span data-ttu-id="987d0-108">Bu `.razor.css` dosya kapsamlı BIR *CSS dosyasıdır*.</span><span class="sxs-lookup"><span data-stu-id="987d0-108">This `.razor.css` file is a *scoped CSS file*.</span></span> 
...
<span data-ttu-id="987d0-125">`MyProjectName.styles.css`Dosya, bir stil bildirimini bileşeniyle gruplamak için kapsam tanımlayıcısını kullanır.</span><span class="sxs-lookup"><span data-stu-id="987d0-125">The `MyProjectName.styles.css` file uses the scope identifier to group a style declaration with its component.</span></span> <span data-ttu-id="987d0-126">Aşağıdaki örnek, önceki öğenin stilini sağlar `<h1>` :</span><span class="sxs-lookup"><span data-stu-id="987d0-126">The following example provides the style for the preceding `<h1>` element:</span></span>
...
## <a name="child-component-support"></a><span data-ttu-id="987d0-131">Alt bileşen desteği</span><span class="sxs-lookup"><span data-stu-id="987d0-131">Child component support</span></span>

<span data-ttu-id="987d0-132">Varsayılan olarak, CSS yalıtımı yalnızca biçimiyle ilişkilendirdiğiniz bileşen için geçerlidir `{COMPONENT NAME}.razor.css` ; burada yer tutucu `{COMPONENT NAME}` genellikle bileşen adıdır.</span><span class="sxs-lookup"><span data-stu-id="987d0-132">By default, CSS isolation only applies to the component you associate with the format `{COMPONENT NAME}.razor.css`, where the placeholder `{COMPONENT NAME}` is usually the component name.</span></span> <span data-ttu-id="987d0-133">Bir alt bileşene değişiklikler uygulamak için, `::deep` Combinator üst bileşenin dosyasındaki tüm alt öğeleri kullanın `.razor.css` .</span><span class="sxs-lookup"><span data-stu-id="987d0-133">To apply changes to a child component, use the `::deep` combinator to any descendant elements in the parent component's `.razor.css` file.</span></span> <span data-ttu-id="987d0-134">`::deep`Combinator, bir öğenin oluşturulan kapsam *descendants* tanımlayıcısının alt öğeleri olan öğeleri seçer.</span><span class="sxs-lookup"><span data-stu-id="987d0-134">The `::deep` combinator selects elements that are *descendants* of an element's generated scope identifier.</span></span> 

<span data-ttu-id="987d0-135">Aşağıdaki örnek, `Parent` adlı bir alt bileşen ile çağrılan bir üst bileşeni gösterir `Child` .</span><span class="sxs-lookup"><span data-stu-id="987d0-135">The following example shows a parent component called `Parent` with a child component called `Child`.</span></span>

<span data-ttu-id="987d0-136">`Parent.razor`:</span><span class="sxs-lookup"><span data-stu-id="987d0-136">`Parent.razor`:</span></span>
...
<span data-ttu-id="987d0-137">`Child.razor`:</span><span class="sxs-lookup"><span data-stu-id="987d0-137">`Child.razor`:</span></span>

```razor
<h1>Child Component</h1>
```

<span data-ttu-id="987d0-138">`h1` `Parent.razor.css` `::deep` `h1` Stil bildiriminin üst bileşene ve alt öğelerine uygulanması gerektiğini belirtmek için Combinator ile içindeki bildirimi güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="987d0-138">Update the `h1` declaration in `Parent.razor.css` with the `::deep` combinator to signify the `h1` style declaration must apply to the parent component and its children:</span></span>
...
<span data-ttu-id="987d0-139">`h1`Stil artık `Parent` `Child` alt bileşen için ayrı bir kapsamlı CSS dosyası oluşturmaya gerek olmadan ve bileşenleri için geçerlidir.</span><span class="sxs-lookup"><span data-stu-id="987d0-139">The `h1` style now applies to the `Parent` and `Child` components without the need to create a separate scoped CSS file for the child component.</span></span>

> [!NOTE]
> <span data-ttu-id="987d0-140">`::deep`Combinator yalnızca alt öğeler ile kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="987d0-140">The `::deep` combinator only works with descendant elements.</span></span> <span data-ttu-id="987d0-141">Aşağıdaki HTML yapısı, `h1` stilleri istenen şekilde bileşenlere uygular:</span><span class="sxs-lookup"><span data-stu-id="987d0-141">The following HTML structure applies the `h1` styles to components as expected:</span></span>
> 
> ```razor
> <div>
>     <h1>Parent</h1>
...
> ```
>
> <span data-ttu-id="987d0-142">Bu senaryoda ASP.NET Core üst bileşenin kapsam tanımlayıcısını `div` öğesine uygular, böylece tarayıcı üst bileşenden stil devralmayı bilir.</span><span class="sxs-lookup"><span data-stu-id="987d0-142">In this scenario, ASP.NET Core applies the parent component's scope identifier to the `div` element, so the browser knows to inherit styles from the parent component.</span></span>
>
> <span data-ttu-id="987d0-143">Ancak, öğesinin hariç tutulması `div` alt ilişkiyi kaldırır ve stil alt bileşene uygulanmaz: **not**</span><span class="sxs-lookup"><span data-stu-id="987d0-143">However, excluding the `div` element removes the descendant relationship, and the style is **not** applied to the child component:</span></span>
>
> ```razor
> <h1>Parent</h1>
>
...
## <a name="css-isolation-configuration"></a><span data-ttu-id="987d0-149">CSS yalıtım yapılandırması</span><span class="sxs-lookup"><span data-stu-id="987d0-149">CSS isolation configuration</span></span>

<span data-ttu-id="987d0-150">CSS yalıtım, kullanıma hazır çalışacak şekilde tasarlanmıştır, ancak mevcut araçlar ya da iş akışlarında bağımlılıklar olduğu gibi bazı gelişmiş senaryolar için yapılandırma sağlar.</span><span class="sxs-lookup"><span data-stu-id="987d0-150">CSS isolation is designed to work out-of-the-box but provides configuration for some advanced scenarios, such as when there are dependencies on existing tools or workflows.</span></span>

### <a name="customize-scope-identifier-format"></a><span data-ttu-id="987d0-151">Kapsam tanımlayıcı biçimini Özelleştir</span><span class="sxs-lookup"><span data-stu-id="987d0-151">Customize scope identifier format</span></span>

<span data-ttu-id="987d0-152">Varsayılan olarak, kapsam tanımlayıcıları biçimini kullanır `b-<10-character-string>` .</span><span class="sxs-lookup"><span data-stu-id="987d0-152">By default, scope identifiers use the format `b-<10-character-string>`.</span></span> <span data-ttu-id="987d0-153">Kapsam tanımlayıcı biçimini özelleştirmek için, proje dosyasını istenen bir düzene güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="987d0-153">To customize the scope identifier format, update the project file to a desired pattern:</span></span>
...
<span data-ttu-id="987d0-156">`scoped.styles.css`Dosya, uygulamanın kökünde oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="987d0-156">The `scoped.styles.css` file is generated at the root of the app.</span></span> <span data-ttu-id="987d0-157">Proje dosyasında, `StaticWebAssetBasePath` varsayılan yolu değiştirmek için özelliğini kullanın.</span><span class="sxs-lookup"><span data-stu-id="987d0-157">In the project file, use the `StaticWebAssetBasePath` property to change the default path.</span></span> <span data-ttu-id="987d0-158">Aşağıdaki örnek, `scoped.styles.css` yolundaki dosyayı ve uygulamanın varlıklarını geri kalanını şu `_content` yolda koyar:</span><span class="sxs-lookup"><span data-stu-id="987d0-158">The following example places the `scoped.styles.css` file, and the rest of the app's assets, at the `_content` path:</span></span>
...
<span data-ttu-id="987d0-166">Aşağıdaki örnekte:</span><span class="sxs-lookup"><span data-stu-id="987d0-166">In the following example:</span></span>

* <span data-ttu-id="987d0-167">Statik Web varlık temel yolu `_content/ClassLib` .</span><span class="sxs-lookup"><span data-stu-id="987d0-167">The static web asset base path is `_content/ClassLib`.</span></span>
* <span data-ttu-id="987d0-168">Sınıf kitaplığının derleme adı `ClassLib` .</span><span class="sxs-lookup"><span data-stu-id="987d0-168">The class library's assembly name is `ClassLib`.</span></span>

```html
<link href="_content/ClassLib/ClassLib.bundle.scp.css" rel="stylesheet">
```

<span data-ttu-id="987d0-169">RCLs ve bileşen kitaplıkları hakkında daha fazla bilgi için bkz.:</span><span class="sxs-lookup"><span data-stu-id="987d0-169">For more information on RCLs and component libraries, see:</span></span>

* <xref:razor-pages/ui-class>
* <span data-ttu-id="987d0-170"><xref:blazor/components/class-libraries>.</span><span class="sxs-lookup"><span data-stu-id="987d0-170"><xref:blazor/components/class-libraries>.</span></span>
