...
1. **Özel etki alanları** dikey penceresine gidin. Etki alanı Kaydedicinizin Web sitesinde, etki alanını yapılandırmak için **IP adresini** ve **özel etkı alanı doğrulama kimliğini** kullanın. Tipik bir etki alanı yapılandırması şunları içerir:
   ...
   * **Ana bilgisayarı**  `asuid` ve Azure tarafından oluşturulan ve Azure Portal tarafından oluşturulan doğrulama kimliği değeri olan bir TXT kaydı.
...
1. Visual Studio 'da *sunucu* projesi için bir Azure App Service [Yayımlama profili](xref:host-and-deploy/visual-studio-publish-profiles#publish-profiles) oluşturun. Menü çubuğundan: **derleme**  >    >  **Yeni**  >  **Azure**  >  **Azure App Service** yayımlama (Windows veya Linux) öğesini seçin. Visual Studio bir Azure aboneliğine bağlıyken, Azure kaynaklarının **görünümünü** **kaynak türüne** göre ayarlayabilirsiniz. Uygulamanın App Service bulmak ve bunu seçmek için **Web uygulaması** listesinde gezinin. **Son**'u seçin.
