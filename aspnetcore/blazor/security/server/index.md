---
title: 'Güvenli ASP.NET Core :::no-loc(Blazor Server)::: uygulamaları'
author: guardrex
description: 'Uygulamaları ASP.NET Core uygulamalar olarak güvenli hale getirme hakkında bilgi edinin :::no-loc(Blazor Server)::: .'
...
---
# <a name="secure-aspnet-core-no-locblazor-server-apps"></a><span data-ttu-id="d3f50-103">Güvenli ASP.NET Core :::no-loc(Blazor Server)::: uygulamaları</span><span class="sxs-lookup"><span data-stu-id="d3f50-103">Secure ASP.NET Core :::no-loc(Blazor Server)::: apps</span></span>
...
<span data-ttu-id="d3f50-105">:::no-loc(Blazor Server)::: uygulamalar, güvenlik için ASP.NET Core uygulamalarla aynı şekilde yapılandırılır.</span><span class="sxs-lookup"><span data-stu-id="d3f50-105">:::no-loc(Blazor Server)::: apps are configured for security in the same manner as ASP.NET Core apps.</span></span> <span data-ttu-id="d3f50-106">Daha fazla bilgi için, altındaki makalelere bakın <xref:security/index> .</span><span class="sxs-lookup"><span data-stu-id="d3f50-106">For more information, see the articles under <xref:security/index>.</span></span> <span data-ttu-id="d3f50-107">Bu genel bakışın altındaki konular özellikle için geçerlidir :::no-loc(Blazor Server)::: .</span><span class="sxs-lookup"><span data-stu-id="d3f50-107">Topics under this overview apply specifically to :::no-loc(Blazor Server):::.</span></span>

## <a name="no-locblazor-server-project-template"></a><span data-ttu-id="d3f50-108">:::no-loc(Blazor Server)::: Proje şablonu</span><span class="sxs-lookup"><span data-stu-id="d3f50-108">:::no-loc(Blazor Server)::: project template</span></span>

<span data-ttu-id="d3f50-109">Proje :::no-loc(Blazor Server)::: oluşturulduğunda proje şablonu kimlik doğrulaması için yapılandırılabilir.</span><span class="sxs-lookup"><span data-stu-id="d3f50-109">The :::no-loc(Blazor Server)::: project template can be configured for authentication when the project is created.</span></span>
...
<span data-ttu-id="d3f50-111"><xref:blazor/tooling>Kimlik doğrulama mekanizmasına sahip yeni bir proje oluşturmak için Içindeki Visual Studio kılavuzunu izleyin :::no-loc(Blazor Server)::: .</span><span class="sxs-lookup"><span data-stu-id="d3f50-111">Follow the Visual Studio guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism.</span></span>

<span data-ttu-id="d3f50-112">**Yeni bir ASP.NET Core Web uygulaması oluştur** iletişim kutusunda **:::no-loc(Blazor Server)::: uygulama** şablonunu seçtikten sonra, **kimlik doğrulaması** altında **Değiştir** ' i seçin.</span><span class="sxs-lookup"><span data-stu-id="d3f50-112">After choosing the **:::no-loc(Blazor Server)::: App** template in the **Create a new ASP.NET Core Web Application** dialog, select **Change** under **Authentication** .</span></span>
...
* <span data-ttu-id="d3f50-115">**Bireysel kullanıcı hesapları** : Kullanıcı hesapları depolanabilir:</span><span class="sxs-lookup"><span data-stu-id="d3f50-115">**Individual User Accounts** : User accounts can be stored:</span></span>
  * <span data-ttu-id="d3f50-116">ASP.NET Core sistemi kullanılarak uygulama içinde [:::no-loc(Identity):::](xref:security/authentication/identity) .</span><span class="sxs-lookup"><span data-stu-id="d3f50-116">Within the app using ASP.NET Core's [:::no-loc(Identity):::](xref:security/authentication/identity) system.</span></span>
...
<span data-ttu-id="d3f50-121"><xref:blazor/tooling>Kimlik doğrulama mekanizmasıyla yeni bir proje oluşturmak için içindeki Visual Studio Code kılavuzunu izleyin :::no-loc(Blazor Server)::: :</span><span class="sxs-lookup"><span data-stu-id="d3f50-121">Follow the Visual Studio Code guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism:</span></span>
...
| `Individual`             | <span data-ttu-id="d3f50-127">Uygulamada depolanan kullanıcılar :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="d3f50-127">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
1. <span data-ttu-id="d3f50-138">**Yeni :::no-loc(Blazor Server)::: uygulamanızı yapılandırın** adımındaki **kimlik doğrulaması** açılan listesinden **bireysel kimlik doğrulaması (uygulama içi)** seçeneğini belirleyin.</span><span class="sxs-lookup"><span data-stu-id="d3f50-138">On the **Configure your new :::no-loc(Blazor Server)::: App** step, select **Individual Authentication (in-app)** from the **Authentication** drop down.</span></span>

1. <span data-ttu-id="d3f50-139">Uygulama, uygulamasında depolanan bireysel kullanıcılar için oluşturulur :::no-loc(ASP.NET Core Identity)::: .</span><span class="sxs-lookup"><span data-stu-id="d3f50-139">The app is created for individual users stored in the app with :::no-loc(ASP.NET Core Identity):::.</span></span>
...
<span data-ttu-id="d3f50-141">Bir :::no-loc(Blazor Server)::: komut kabuğunda aşağıdaki komutu kullanarak kimlik doğrulama mekanizması ile yeni bir proje oluşturun:</span><span class="sxs-lookup"><span data-stu-id="d3f50-141">Create a new :::no-loc(Blazor Server)::: project with an authentication mechanism using the following command in a command shell:</span></span>
...
| `Individual`             | <span data-ttu-id="d3f50-147">Uygulamada depolanan kullanıcılar :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="d3f50-147">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
* <span data-ttu-id="d3f50-157">:::no-loc(Blazor Server):::Bir komut kabuğu 'nda () şablonu için Yardım komutunu yürütün `blazorserver` :</span><span class="sxs-lookup"><span data-stu-id="d3f50-157">Execute the help command for the :::no-loc(Blazor Server)::: template (`blazorserver`) in a command shell:</span></span>
...
## <a name="scaffold-no-locidentity"></a><span data-ttu-id="d3f50-158">İskele :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="d3f50-158">Scaffold :::no-loc(Identity):::</span></span>

<span data-ttu-id="d3f50-159">:::no-loc(Identity):::Bir projeye yapı iskelesi :::no-loc(Blazor Server)::: :</span><span class="sxs-lookup"><span data-stu-id="d3f50-159">Scaffold :::no-loc(Identity)::: into a :::no-loc(Blazor Server)::: project:</span></span>
