...
---
# <a name="aspnet-core-no-locblazor-hosting-model-configuration"></a><span data-ttu-id="17d4d-103">ASP.NET Core :::no-loc(Blazor)::: barındırma modeli yapılandırması</span><span class="sxs-lookup"><span data-stu-id="17d4d-103">ASP.NET Core :::no-loc(Blazor)::: hosting model configuration</span></span>
...
### <a name="no-locsignalr-cross-origin-negotiation-for-authentication"></a><span data-ttu-id="17d4d-106">:::no-loc(SignalR)::: kimlik doğrulaması için çıkış noktaları arası anlaşma</span><span class="sxs-lookup"><span data-stu-id="17d4d-106">:::no-loc(SignalR)::: cross-origin negotiation for authentication</span></span>

<span data-ttu-id="17d4d-107">*Bu bölüm için geçerlidir :::no-loc(Blazor WebAssembly)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-107">*This section applies to :::no-loc(Blazor WebAssembly):::.*</span></span>

<span data-ttu-id="17d4d-108">:::no-loc(SignalR):::Temel alınan istemciyi, :::no-loc(cookie)::: s veya http kimlik doğrulama üstbilgileri gibi kimlik bilgilerini gönderecek şekilde yapılandırmak için:</span><span class="sxs-lookup"><span data-stu-id="17d4d-108">To configure :::no-loc(SignalR):::'s underlying client to send credentials, such as :::no-loc(cookie):::s or HTTP authentication headers:</span></span>
...
<span data-ttu-id="17d4d-113">*Bu bölüm için geçerlidir :::no-loc(Blazor Server)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-113">*This section applies to :::no-loc(Blazor Server):::.*</span></span>
...
<span data-ttu-id="17d4d-116">Kullanıcı arabirimini özelleştirmek için, sayfanın içindeki öğesiyle bir öğe `id` tanımlayın `components-reconnect-modal` `<body>` `_Host.cshtml` :::no-loc(Razor)::: :</span><span class="sxs-lookup"><span data-stu-id="17d4d-116">To customize the UI, define an element with an `id` of `components-reconnect-modal` in the `<body>` of the `_Host.cshtml` :::no-loc(Razor)::: page:</span></span>
...
| `components-reconnect-failed`   | <span data-ttu-id="17d4d-126">Muhtemelen bir ağ hatasından dolayı yeniden bağlantı başarısız oldu.</span><span class="sxs-lookup"><span data-stu-id="17d4d-126">Reconnection failed, probably due to a network failure.</span></span> <span data-ttu-id="17d4d-127">Yeniden bağlanmayı denemek için çağrısı yapın `window.:::no-loc(Blazor):::.reconnect()` .</span><span class="sxs-lookup"><span data-stu-id="17d4d-127">To attempt reconnection, call `window.:::no-loc(Blazor):::.reconnect()`.</span></span> |
...
<span data-ttu-id="17d4d-137">*Bu bölüm, barındırılan ve için geçerlidir :::no-loc(Blazor WebAssembly)::: :::no-loc(Blazor Server)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-137">*This section applies to hosted :::no-loc(Blazor WebAssembly)::: and :::no-loc(Blazor Server):::.*</span></span>

<span data-ttu-id="17d4d-138">:::no-loc(Blazor)::: uygulamalar varsayılan olarak, sunucudaki kullanıcı arabirimini PreRender 'a göre ayarlanır.</span><span class="sxs-lookup"><span data-stu-id="17d4d-138">:::no-loc(Blazor)::: apps are set up by default to prerender the UI on the server.</span></span> <span data-ttu-id="17d4d-139">Daha fazla bilgi için bkz. <xref:mvc/views/tag-helpers/builtin-th/component-tag-helper>.</span><span class="sxs-lookup"><span data-stu-id="17d4d-139">For more information, see <xref:mvc/views/tag-helpers/builtin-th/component-tag-helper>.</span></span>

::: moniker-end

::: moniker range="< aspnetcore-5.0"

<span data-ttu-id="17d4d-140">*Bu bölüm için geçerlidir :::no-loc(Blazor Server)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-140">*This section applies to :::no-loc(Blazor Server):::.*</span></span>

<span data-ttu-id="17d4d-141">:::no-loc(Blazor Server)::: uygulamalar, sunucu bağlantısı oluşturulmadan önce sunucudaki kullanıcı arabirimini varsayılan olarak PreRender 'a ayarlar.</span><span class="sxs-lookup"><span data-stu-id="17d4d-141">:::no-loc(Blazor Server)::: apps are set up by default to prerender the UI on the server before the client connection to the server is established.</span></span> <span data-ttu-id="17d4d-142">Daha fazla bilgi için bkz. <xref:mvc/views/tag-helpers/builtin-th/component-tag-helper>.</span><span class="sxs-lookup"><span data-stu-id="17d4d-142">For more information, see <xref:mvc/views/tag-helpers/builtin-th/component-tag-helper>.</span></span>

::: moniker-end

## <a name="initialize-the-no-locblazor-circuit"></a><span data-ttu-id="17d4d-143">:::no-loc(Blazor):::Devresini başlatma</span><span class="sxs-lookup"><span data-stu-id="17d4d-143">Initialize the :::no-loc(Blazor)::: circuit</span></span>

<span data-ttu-id="17d4d-144">*Bu bölüm için geçerlidir :::no-loc(Blazor Server)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-144">*This section applies to :::no-loc(Blazor Server):::.*</span></span>

<span data-ttu-id="17d4d-145">:::no-loc(Blazor Server):::Dosyada uygulamanın [ :::no-loc(SignalR)::: devresine](xref:blazor/hosting-models#circuits) el ile başlangıcını yapılandırın `Pages/_Host.cshtml` :</span><span class="sxs-lookup"><span data-stu-id="17d4d-145">Configure the manual start of a :::no-loc(Blazor Server)::: app's [:::no-loc(SignalR)::: circuit](xref:blazor/hosting-models#circuits) in the `Pages/_Host.cshtml` file:</span></span>
...
* <span data-ttu-id="17d4d-147">`:::no-loc(Blazor):::.start` `blazor.server.js` Komut dosyasının etiketinden ve kapanış etiketinin içindeyken çağıran bir betik yerleştirin `</body>` .</span><span class="sxs-lookup"><span data-stu-id="17d4d-147">Place a script that calls `:::no-loc(Blazor):::.start` after the `blazor.server.js` script's tag and inside the closing `</body>` tag.</span></span>

<span data-ttu-id="17d4d-148">`autostart`Devre dışı bırakıldığında, devresine bağımlı olmayan uygulamanın herhangi bir yönü normal şekilde çalışır.</span><span class="sxs-lookup"><span data-stu-id="17d4d-148">When `autostart` is disabled, any aspect of the app that doesn't depend on the circuit works normally.</span></span> <span data-ttu-id="17d4d-149">Örneğin, istemci tarafı yönlendirme çalışır.</span><span class="sxs-lookup"><span data-stu-id="17d4d-149">For example, client-side routing is operational.</span></span> <span data-ttu-id="17d4d-150">Ancak, devresine bağlı olan herhangi bir boyut, `:::no-loc(Blazor):::.start` çağrılana kadar çalışmaz.</span><span class="sxs-lookup"><span data-stu-id="17d4d-150">However, any aspect that depends on the circuit isn't operational until `:::no-loc(Blazor):::.start` is called.</span></span> <span data-ttu-id="17d4d-151">Uygulama davranışı, kurulu bir devre olmadan tahmin edilemez.</span><span class="sxs-lookup"><span data-stu-id="17d4d-151">App behavior is unpredictable without an established circuit.</span></span> <span data-ttu-id="17d4d-152">Örneğin, devre kesildiğinde bileşen yöntemleri yürütülemeyebilir.</span><span class="sxs-lookup"><span data-stu-id="17d4d-152">For example, component methods fail to execute while the circuit is disconnected.</span></span>

### <a name="initialize-no-locblazor-when-the-document-is-ready"></a><span data-ttu-id="17d4d-153">:::no-loc(Blazor):::Belge hazır olduğunda Başlat</span><span class="sxs-lookup"><span data-stu-id="17d4d-153">Initialize :::no-loc(Blazor)::: when the document is ready</span></span>

<span data-ttu-id="17d4d-154">:::no-loc(Blazor):::Belge hazır olduğunda uygulamayı başlatmak için:</span><span class="sxs-lookup"><span data-stu-id="17d4d-154">To initialize the :::no-loc(Blazor)::: app when the document is ready:</span></span>
...
<span data-ttu-id="17d4d-156">JS birlikte çalışma başlatma gibi ek görevler gerçekleştirmek için, `then` `Promise` el ile gerçekleştirilen bir uygulama başlangıcını kullanarak bu sonuçlara zincir atamak için kullanın :::no-loc(Blazor)::: :</span><span class="sxs-lookup"><span data-stu-id="17d4d-156">To perform additional tasks, such as JS interop initialization, use `then` to chain to the `Promise` that results from a manual :::no-loc(Blazor)::: app start:</span></span>
...
### <a name="configure-the-no-locsignalr-client"></a><span data-ttu-id="17d4d-157">İstemciyi yapılandırma :::no-loc(SignalR):::</span><span class="sxs-lookup"><span data-stu-id="17d4d-157">Configure the :::no-loc(SignalR)::: client</span></span>
...
<span data-ttu-id="17d4d-159">:::no-loc(SignalR):::İstemci günlüğünü yapılandırmak için, `configure:::no-loc(SignalR):::` `configureLogging` istemci Oluşturucu 'da günlük düzeyiyle çağıran bir yapılandırma nesnesi () geçirin:</span><span class="sxs-lookup"><span data-stu-id="17d4d-159">To configure :::no-loc(SignalR)::: client logging, pass in a configuration object (`configure:::no-loc(SignalR):::`) that calls `configureLogging` with the log level on the client builder:</span></span>
...
## <a name="disconnect-the-no-locblazor-circuit-from-the-client"></a><span data-ttu-id="17d4d-177">:::no-loc(Blazor):::Devresini istemciden bağlantısını kesin</span><span class="sxs-lookup"><span data-stu-id="17d4d-177">Disconnect the :::no-loc(Blazor)::: circuit from the client</span></span>

<span data-ttu-id="17d4d-178">Varsayılan olarak, :::no-loc(Blazor)::: [ `unload` sayfa etkinliği](https://developer.mozilla.org/docs/Web/API/Window/unload_event) tetiklendiğinde devre dışı bırakıldığında bağlantı kesilir.</span><span class="sxs-lookup"><span data-stu-id="17d4d-178">By default, a :::no-loc(Blazor)::: circuit is disconnected when the [`unload` page event](https://developer.mozilla.org/docs/Web/API/Window/unload_event) is triggered.</span></span> <span data-ttu-id="17d4d-179">Bu bağlantı devresinin, istemcideki diğer senaryolara karşı bağlantısını kesmek için `:::no-loc(Blazor):::.disconnect` uygun olay işleyicisini çağırın.</span><span class="sxs-lookup"><span data-stu-id="17d4d-179">To disconnect the circuit for other scenarios on the client, invoke `:::no-loc(Blazor):::.disconnect` in the appropriate event handler.</span></span> <span data-ttu-id="17d4d-180">Aşağıdaki örnekte, sayfa gizli olduğunda devre dışı bırakıldığında bağlantı kesilir ([ `pagehide` olay](https://developer.mozilla.org/docs/Web/API/Window/pagehide_event)):</span><span class="sxs-lookup"><span data-stu-id="17d4d-180">In the following example, the circuit is disconnected when the page is hidden ([`pagehide` event](https://developer.mozilla.org/docs/Web/API/Window/pagehide_event)):</span></span>
...
<span data-ttu-id="17d4d-182">*Bu bölüm için geçerlidir :::no-loc(Blazor Server)::: .*</span><span class="sxs-lookup"><span data-stu-id="17d4d-182">*This section applies to :::no-loc(Blazor Server):::.*</span></span>
...
* <span data-ttu-id="17d4d-188"><xref:Microsoft.AspNetCore.Builder.StaticFileExtensions.UseStaticFiles%2A>() İçinde için iki çağrı kullanın `Startup.Configure` `Startup.cs` :</span><span class="sxs-lookup"><span data-stu-id="17d4d-188">Use two calls to <xref:Microsoft.AspNetCore.Builder.StaticFileExtensions.UseStaticFiles%2A> in `Startup.Configure` (`Startup.cs`):</span></span>
  ...
  * <span data-ttu-id="17d4d-190">İkinci ara yazılım, `blazor.server.js` Framework tarafından sunulan varsayılan statik dosya yapılandırmasını kullanan hizmet verir :::no-loc(Blazor)::: .</span><span class="sxs-lookup"><span data-stu-id="17d4d-190">The second middleware serves `blazor.server.js`, which uses the default static files configuration provided by the :::no-loc(Blazor)::: framework.</span></span>
...
* [<span data-ttu-id="17d4d-193">:::no-loc(Blazor Server)::: yeniden bağlanma olayları ve bileşen yaşam döngüsü olayları</span><span class="sxs-lookup"><span data-stu-id="17d4d-193">:::no-loc(Blazor Server)::: reconnection events and component lifecycle events</span></span>](xref:blazor/components/lifecycle#blazor-server-reconnection-events)
