...
<span data-ttu-id="86be3-107">Şu durumlarda bir eşzamanlılık çakışması oluşur:</span><span class="sxs-lookup"><span data-stu-id="86be3-107">A concurrency conflict occurs when:</span></span>
...
* <span data-ttu-id="86be3-140">John 'un değişikliğini kemal 'in değişikliğini geçersiz kılabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="86be3-140">You can let John's change overwrite Jane's change.</span></span>
...
<span data-ttu-id="86be3-170">*Modeller/departman. cs*' de, rowversion adlı bir izleme özelliği ekleyin:</span><span class="sxs-lookup"><span data-stu-id="86be3-170">In *Models/Department.cs*, add a tracking property named RowVersion:</span></span>
...
* <span data-ttu-id="86be3-196">Geçerli satır sürümü değeri getirilen değerle eşleşmiyor.</span><span class="sxs-lookup"><span data-stu-id="86be3-196">The current row version value doesn't match the fetched value.</span></span>
...
* <span data-ttu-id="86be3-207">*Geçişleri/{zaman damgası} _RowVersion. cs* geçiş dosyasını oluşturur.</span><span class="sxs-lookup"><span data-stu-id="86be3-207">Creates the *Migrations/{time stamp}_RowVersion.cs* migration file.</span></span>
...
<span data-ttu-id="86be3-270">İki tarayıcı sekmesi aynı bilgileri görüntüler.</span><span class="sxs-lookup"><span data-stu-id="86be3-270">The two browser tabs display the same information.</span></span>
...
### <a name="test-concurrency-conflicts"></a><span data-ttu-id="86be3-304">Eşzamanlılık çakışmalarını test et</span><span class="sxs-lookup"><span data-stu-id="86be3-304">Test concurrency conflicts</span></span>
...
## <a name="next-steps"></a><span data-ttu-id="86be3-320">Sonraki adımlar</span><span class="sxs-lookup"><span data-stu-id="86be3-320">Next steps</span></span>
...
### <a name="optimistic-concurrency"></a><span data-ttu-id="86be3-335">İyimser eşzamanlılık</span><span class="sxs-lookup"><span data-stu-id="86be3-335">Optimistic concurrency</span></span>
...
* <span data-ttu-id="86be3-346">Bir kullanıcının hangi özelliği değiştirmiş olduğunu izleyebilir ve yalnızca VERITABANıNDAKI ilgili sütunları güncelleştirebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="86be3-346">You can keep track of which property a user has modified and update only the corresponding columns in the DB.</span></span>
  ...
  * <span data-ttu-id="86be3-352">Aynı özellikte rekabet değişiklikleri yapılmışsa veri kaybını önleyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="86be3-352">Can't avoid data loss if competing changes are made to the same property.</span></span>
...
## <a name="handling-concurrency"></a><span data-ttu-id="86be3-369">Eşzamanlılık işleme</span><span class="sxs-lookup"><span data-stu-id="86be3-369">Handling concurrency</span></span> 
...
### <a name="detecting-concurrency-conflicts-on-a-row"></a><span data-ttu-id="86be3-380">Bir satırda eşzamanlılık çakışmalarını algılama</span><span class="sxs-lookup"><span data-stu-id="86be3-380">Detecting concurrency conflicts on a row</span></span>
...
<span data-ttu-id="86be3-406">Özelliği eklendiğinde `RowVersion` , geçiş GEREKTIREN DB modeli değişir.</span><span class="sxs-lookup"><span data-stu-id="86be3-406">Adding the `RowVersion` property changes the DB model, which requires a migration.</span></span>
...
# <a name="visual-studio-code"></a>[<span data-ttu-id="86be3-417">Visual Studio Code</span><span class="sxs-lookup"><span data-stu-id="86be3-417">Visual Studio Code</span></span>](#tab/visual-studio-code)
 ...
 <span data-ttu-id="86be3-418">Şu komutu çalıştırın:</span><span class="sxs-lookup"><span data-stu-id="86be3-418">Run the following command:</span></span>
...
<span data-ttu-id="86be3-433">Aşağıdaki kodla *Pages\Departments\Edit.cshtml.cs* güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="86be3-433">Update *Pages\Departments\Edit.cshtml.cs* with the following code:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Edit.cshtml.cs?name=snippet)]

<span data-ttu-id="86be3-434">Bir eşzamanlılık sorunu tespit etmek için [OriginalValue](/dotnet/api/microsoft.entityframeworkcore.changetracking.propertyentry.originalvalue?view=efcore-2.0#Microsoft_EntityFrameworkCore_ChangeTracking_PropertyEntry_OriginalValue) , `rowVersion` getirilen varlıktaki değeri ile güncelleştirilir.</span><span class="sxs-lookup"><span data-stu-id="86be3-434">To detect a concurrency issue, the [OriginalValue](/dotnet/api/microsoft.entityframeworkcore.changetracking.propertyentry.originalvalue?view=efcore-2.0#Microsoft_EntityFrameworkCore_ChangeTracking_PropertyEntry_OriginalValue) is updated with the `rowVersion` value from the entity it was fetched.</span></span> <span data-ttu-id="86be3-435">EF Core, özgün değeri içeren WHERE yan tümcesiyle bir SQL UPDATE komutu oluşturur `RowVersion` .</span><span class="sxs-lookup"><span data-stu-id="86be3-435">EF Core generates a SQL UPDATE command with a WHERE clause containing the original `RowVersion` value.</span></span> <span data-ttu-id="86be3-436">UPDATE komutundan hiçbir satır etkilenmeden (hiçbir satır özgün değere sahip değilse `RowVersion` ), bir `DbUpdateConcurrencyException` özel durum oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="86be3-436">If no rows are affected by the UPDATE command (no rows have the original `RowVersion` value), a `DbUpdateConcurrencyException` exception is thrown.</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Edit.cshtml.cs?name=snippet_rv&highlight=24-999)]

<span data-ttu-id="86be3-437">Önceki kodda, `Department.RowVersion` varlık getirilirken değeridir.</span><span class="sxs-lookup"><span data-stu-id="86be3-437">In the preceding code, `Department.RowVersion` is the value when the entity was fetched.</span></span> <span data-ttu-id="86be3-438">`OriginalValue` Bu yöntemde çağrıldığında, DB 'deki değerdir `FirstOrDefaultAsync` .</span><span class="sxs-lookup"><span data-stu-id="86be3-438">`OriginalValue` is the value in the DB when `FirstOrDefaultAsync` was called in this method.</span></span>

<span data-ttu-id="86be3-439">Aşağıdaki kod, istemci değerlerini (Bu yönteme gönderilen değerler) ve DB değerlerini alır:</span><span class="sxs-lookup"><span data-stu-id="86be3-439">The following code gets the client values (the values posted to this method) and the DB values:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Edit.cshtml.cs?name=snippet_try&highlight=9,18)]

<span data-ttu-id="86be3-440">Aşağıdaki kod, ' den postalandıklarından farklı olarak DB değerleri olan her bir sütun için özel bir hata iletisi ekler `OnPostAsync` :</span><span class="sxs-lookup"><span data-stu-id="86be3-440">The following code adds a custom error message for each column that has DB values different from what was posted to `OnPostAsync`:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Edit.cshtml.cs?name=snippet_err)]

<span data-ttu-id="86be3-441">Aşağıdaki vurgulanan kod, `RowVersion` değeri, veritabanından alınan yeni değer olarak ayarlar.</span><span class="sxs-lookup"><span data-stu-id="86be3-441">The following highlighted code sets the `RowVersion` value to the new value retrieved from the DB.</span></span> <span data-ttu-id="86be3-442">Kullanıcının bir dahaki sefer **Kaydet**' i tıklatması durumunda, yalnızca düzenleme sayfasının son görüntüsü bu yana gerçekleşen eşzamanlılık hataları yakalanacaktır.</span><span class="sxs-lookup"><span data-stu-id="86be3-442">The next time the user clicks **Save**, only concurrency errors that happen since the last display of the Edit page will be caught.</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Edit.cshtml.cs?name=snippet_try&highlight=23)]

<span data-ttu-id="86be3-443">`ModelState.Remove`Eski değere sahip olduğundan deyimin olması gerekir `ModelState` `RowVersion` .</span><span class="sxs-lookup"><span data-stu-id="86be3-443">The `ModelState.Remove` statement is required because `ModelState` has the old `RowVersion` value.</span></span> <span data-ttu-id="86be3-444">RazorSayfada, `ModelState` bir alanın değeri, her ikisi de varsa Model Özellik değerlerinden önceliklidir.</span><span class="sxs-lookup"><span data-stu-id="86be3-444">In the Razor Page, the `ModelState` value for a field takes precedence over the model property values when both are present.</span></span>

## <a name="update-the-edit-page"></a><span data-ttu-id="86be3-445">Düzenleme sayfasını Güncelleştir</span><span class="sxs-lookup"><span data-stu-id="86be3-445">Update the Edit page</span></span>

<span data-ttu-id="86be3-446">*Sayfaları/departmanları/Düzenle. cshtml* 'yi şu biçimlendirmeyle güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="86be3-446">Update *Pages/Departments/Edit.cshtml* with the following markup:</span></span>

[!code-cshtml[](intro/samples/cu/Pages/Departments/Edit.cshtml?highlight=1,14,16-17,37-39)]

<span data-ttu-id="86be3-447">Yukarıdaki biçimlendirme:</span><span class="sxs-lookup"><span data-stu-id="86be3-447">The preceding markup:</span></span>

* <span data-ttu-id="86be3-448">`page`İçindeki yönergesini ' a `@page` güncelleştirir `@page "{id:int}"` .</span><span class="sxs-lookup"><span data-stu-id="86be3-448">Updates the `page` directive from `@page` to `@page "{id:int}"`.</span></span>
* <span data-ttu-id="86be3-449">Gizli satır sürümü ekler.</span><span class="sxs-lookup"><span data-stu-id="86be3-449">Adds a hidden row version.</span></span> <span data-ttu-id="86be3-450">`RowVersion` , geri gönder değeri bağlayan için eklenmelidir.</span><span class="sxs-lookup"><span data-stu-id="86be3-450">`RowVersion` must be added so post back binds the value.</span></span>
* <span data-ttu-id="86be3-451">`RowVersion`Hata ayıklama amacıyla son baytını görüntüler.</span><span class="sxs-lookup"><span data-stu-id="86be3-451">Displays the last byte of `RowVersion` for debugging purposes.</span></span>
* <span data-ttu-id="86be3-452">`ViewData`Türü kesin belirlenmiş olan ile değiştirilir `InstructorNameSL` .</span><span class="sxs-lookup"><span data-stu-id="86be3-452">Replaces `ViewData` with the strongly-typed `InstructorNameSL`.</span></span>

## <a name="test-concurrency-conflicts-with-the-edit-page"></a><span data-ttu-id="86be3-453">Düzenleme sayfasıyla eşzamanlılık çakışmalarını test etme</span><span class="sxs-lookup"><span data-stu-id="86be3-453">Test concurrency conflicts with the Edit page</span></span>

<span data-ttu-id="86be3-454">Ingilizce bölümünde düzenleme öğesinin iki tarayıcı örneğini açın:</span><span class="sxs-lookup"><span data-stu-id="86be3-454">Open two browsers instances of Edit on the English department:</span></span>

* <span data-ttu-id="86be3-455">Uygulamayı çalıştırın ve Departmanlar ' ı seçin.</span><span class="sxs-lookup"><span data-stu-id="86be3-455">Run the app and select Departments.</span></span>
* <span data-ttu-id="86be3-456">Ingilizce departman için **düzenleme** köprüsüne sağ tıklayın ve **Yeni sekmede aç**' ı seçin.</span><span class="sxs-lookup"><span data-stu-id="86be3-456">Right-click the **Edit** hyperlink for the English department and select **Open in new tab**.</span></span>
* <span data-ttu-id="86be3-457">İlk sekmede, Ingilizce bölümünün **düzenleme** Köprüsü ' ne tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-457">In the first tab, click the **Edit** hyperlink for the English department.</span></span>

<span data-ttu-id="86be3-458">İki tarayıcı sekmesi aynı bilgileri görüntüler.</span><span class="sxs-lookup"><span data-stu-id="86be3-458">The two browser tabs display the same information.</span></span>

<span data-ttu-id="86be3-459">İlk tarayıcı sekmesinde adı değiştirin ve **Kaydet**' e tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-459">Change the name in the first browser tab and click **Save**.</span></span>

![Bölüm Düzenle sayfa 1 değişiklikten sonra](concurrency/_static/edit-after-change-1.png)

<span data-ttu-id="86be3-461">Tarayıcı, değiştirilen değeri olan dizin sayfasını gösterir ve rowVersion göstergesi güncellenir.</span><span class="sxs-lookup"><span data-stu-id="86be3-461">The browser shows the Index page with the changed value and updated rowVersion indicator.</span></span> <span data-ttu-id="86be3-462">Güncelleştirilmiş ROWVERSION göstergesinin, diğer sekmede ikinci geri göndermede görüntülendiğini aklınızda edin.</span><span class="sxs-lookup"><span data-stu-id="86be3-462">Note the updated rowVersion indicator, it's displayed on the second postback in the other tab.</span></span>

<span data-ttu-id="86be3-463">İkinci tarayıcı sekmesinden farklı bir alanı değiştirin.</span><span class="sxs-lookup"><span data-stu-id="86be3-463">Change a different field in the second browser tab.</span></span>

![Değişiklik sonrasında bölüm düzenleme sayfası 2](concurrency/_static/edit-after-change-2.png)

<span data-ttu-id="86be3-465">**Kaydet**’e tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-465">Click **Save**.</span></span> <span data-ttu-id="86be3-466">DB değerleriyle eşleşmeyen tüm alanlar için hata iletileri görürsünüz:</span><span class="sxs-lookup"><span data-stu-id="86be3-466">You see error messages for all fields that don't match the DB values:</span></span>

![Bölüm düzenleme sayfası hata iletisi](concurrency/_static/edit-error.png)

<span data-ttu-id="86be3-468">Bu tarayıcı penceresi, ad alanını değiştirmeyi planlamadı.</span><span class="sxs-lookup"><span data-stu-id="86be3-468">This browser window didn't intend to change the Name field.</span></span> <span data-ttu-id="86be3-469">Geçerli değeri (diller) kopyalayın ve ad alanına yapıştırın.</span><span class="sxs-lookup"><span data-stu-id="86be3-469">Copy and paste the current value (Languages) into the Name field.</span></span> <span data-ttu-id="86be3-470">Sekme genişletme. İstemci tarafı doğrulaması hata iletisini kaldırır.</span><span class="sxs-lookup"><span data-stu-id="86be3-470">Tab out. Client-side validation removes the error message.</span></span>

![Bölüm düzenleme sayfası hata iletisi](concurrency/_static/cv.png)

<span data-ttu-id="86be3-472">Yeniden **Kaydet** ' e tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-472">Click **Save** again.</span></span> <span data-ttu-id="86be3-473">İkinci tarayıcı sekmesine girdiğiniz değer kaydedilir.</span><span class="sxs-lookup"><span data-stu-id="86be3-473">The value you entered in the second browser tab is saved.</span></span> <span data-ttu-id="86be3-474">Kaydedilen değerleri Dizin sayfasında görürsünüz.</span><span class="sxs-lookup"><span data-stu-id="86be3-474">You see the saved values in the Index page.</span></span>

## <a name="update-the-delete-page"></a><span data-ttu-id="86be3-475">Silme sayfasını Güncelleştir</span><span class="sxs-lookup"><span data-stu-id="86be3-475">Update the Delete page</span></span>

<span data-ttu-id="86be3-476">Silme sayfası modelini aşağıdaki kodla güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="86be3-476">Update the Delete page model with the following code:</span></span>

[!code-csharp[](intro/samples/cu/Pages/Departments/Delete.cshtml.cs)]

<span data-ttu-id="86be3-477">Sil sayfası, varlık alındıktan sonra değiştirildiğinde eşzamanlılık çakışmalarını algılar.</span><span class="sxs-lookup"><span data-stu-id="86be3-477">The Delete page detects concurrency conflicts when the entity has changed after it was fetched.</span></span> <span data-ttu-id="86be3-478">`Department.RowVersion` , varlık getirilen satır sürümüdür.</span><span class="sxs-lookup"><span data-stu-id="86be3-478">`Department.RowVersion` is the row version when the entity was fetched.</span></span> <span data-ttu-id="86be3-479">EF Core, SQL DELETE komutunu oluşturduğunda, içeren bir WHERE yan tümcesi içerir `RowVersion` .</span><span class="sxs-lookup"><span data-stu-id="86be3-479">When EF Core creates the SQL DELETE command, it includes a WHERE clause with `RowVersion`.</span></span> <span data-ttu-id="86be3-480">SQL DELETE komutu sıfır satırla etkileniyorsa:</span><span class="sxs-lookup"><span data-stu-id="86be3-480">If the SQL DELETE command results in zero rows affected:</span></span>

* <span data-ttu-id="86be3-481">`RowVersion`SQL DELETE komutunda bulunan, DB ile eşleşmiyor `RowVersion` .</span><span class="sxs-lookup"><span data-stu-id="86be3-481">The `RowVersion` in the SQL DELETE command doesn't match `RowVersion` in the DB.</span></span>
* <span data-ttu-id="86be3-482">Bir DbUpdateConcurrencyException özel durumu oluşturulur.</span><span class="sxs-lookup"><span data-stu-id="86be3-482">A DbUpdateConcurrencyException exception is thrown.</span></span>
* <span data-ttu-id="86be3-483">`OnGetAsync` ile çağırılır `concurrencyError` .</span><span class="sxs-lookup"><span data-stu-id="86be3-483">`OnGetAsync` is called with the `concurrencyError`.</span></span>

### <a name="update-the-delete-page"></a><span data-ttu-id="86be3-484">Silme sayfasını Güncelleştir</span><span class="sxs-lookup"><span data-stu-id="86be3-484">Update the Delete page</span></span>

<span data-ttu-id="86be3-485">*Sayfaları/departmanları/delete. cshtml* 'yi aşağıdaki kodla güncelleştirin:</span><span class="sxs-lookup"><span data-stu-id="86be3-485">Update *Pages/Departments/Delete.cshtml* with the following code:</span></span>

[!code-cshtml[](intro/samples/cu/Pages/Departments/Delete.cshtml?highlight=1,10,39,51)]

<span data-ttu-id="86be3-486">Yukarıdaki kod aşağıdaki değişiklikleri yapar:</span><span class="sxs-lookup"><span data-stu-id="86be3-486">The preceding code makes the following changes:</span></span>

* <span data-ttu-id="86be3-487">`page`İçindeki yönergesini ' a `@page` güncelleştirir `@page "{id:int}"` .</span><span class="sxs-lookup"><span data-stu-id="86be3-487">Updates the `page` directive from `@page` to `@page "{id:int}"`.</span></span>
* <span data-ttu-id="86be3-488">Bir hata iletisi ekler.</span><span class="sxs-lookup"><span data-stu-id="86be3-488">Adds an error message.</span></span>
* <span data-ttu-id="86be3-489">FirstMidName öğesini, **yönetici** alanındaki FullName ile değiştirir.</span><span class="sxs-lookup"><span data-stu-id="86be3-489">Replaces FirstMidName with FullName in the **Administrator** field.</span></span>
* <span data-ttu-id="86be3-490">`RowVersion`Son baytın görüntüleneceği değişiklikler.</span><span class="sxs-lookup"><span data-stu-id="86be3-490">Changes `RowVersion` to display the last byte.</span></span>
* <span data-ttu-id="86be3-491">Gizli satır sürümü ekler.</span><span class="sxs-lookup"><span data-stu-id="86be3-491">Adds a hidden row version.</span></span> <span data-ttu-id="86be3-492">`RowVersion` , geri gönder değeri bağlayan için eklenmelidir.</span><span class="sxs-lookup"><span data-stu-id="86be3-492">`RowVersion` must be added so post back binds the value.</span></span>

### <a name="test-concurrency-conflicts-with-the-delete-page"></a><span data-ttu-id="86be3-493">Silme sayfasıyla eşzamanlılık çakışmalarını test etme</span><span class="sxs-lookup"><span data-stu-id="86be3-493">Test concurrency conflicts with the Delete page</span></span>

<span data-ttu-id="86be3-494">Test departmanı oluşturun.</span><span class="sxs-lookup"><span data-stu-id="86be3-494">Create a test department.</span></span>

<span data-ttu-id="86be3-495">Test departmanı üzerinde silmenin iki tarayıcı örneğini açın:</span><span class="sxs-lookup"><span data-stu-id="86be3-495">Open two browsers instances of Delete on the test department:</span></span>

* <span data-ttu-id="86be3-496">Uygulamayı çalıştırın ve Departmanlar ' ı seçin.</span><span class="sxs-lookup"><span data-stu-id="86be3-496">Run the app and select Departments.</span></span>
* <span data-ttu-id="86be3-497">Test departmanı için **silme** köprüsünü sağ tıklayın ve **Yeni sekmede aç**' ı seçin.</span><span class="sxs-lookup"><span data-stu-id="86be3-497">Right-click the **Delete** hyperlink for the test department and select **Open in new tab**.</span></span>
* <span data-ttu-id="86be3-498">Test departmanı için **düzenleme** köprüsüne tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-498">Click the **Edit** hyperlink for the test department.</span></span>

<span data-ttu-id="86be3-499">İki tarayıcı sekmesi aynı bilgileri görüntüler.</span><span class="sxs-lookup"><span data-stu-id="86be3-499">The two browser tabs display the same information.</span></span>

<span data-ttu-id="86be3-500">İlk tarayıcı sekmesinde bütçeyi değiştirin ve **Kaydet**' e tıklayın.</span><span class="sxs-lookup"><span data-stu-id="86be3-500">Change the budget in the first browser tab and click **Save**.</span></span>

<span data-ttu-id="86be3-501">Tarayıcı, değiştirilen değeri olan dizin sayfasını gösterir ve rowVersion göstergesi güncellenir.</span><span class="sxs-lookup"><span data-stu-id="86be3-501">The browser shows the Index page with the changed value and updated rowVersion indicator.</span></span> <span data-ttu-id="86be3-502">Güncelleştirilmiş ROWVERSION göstergesinin, diğer sekmede ikinci geri göndermede görüntülendiğini aklınızda edin.</span><span class="sxs-lookup"><span data-stu-id="86be3-502">Note the updated rowVersion indicator, it's displayed on the second postback in the other tab.</span></span>

<span data-ttu-id="86be3-503">İkinci sekmeden test departmanını silin. Bir eşzamanlılık hatası, VERITABANıNDAKI geçerli değerlerle birlikte görüntülenir.</span><span class="sxs-lookup"><span data-stu-id="86be3-503">Delete the test department from the second tab. A concurrency error is display with the current values from the DB.</span></span> <span data-ttu-id="86be3-504">**Sil** ' i tıklatmak, güncelleştirilmemiş olmadığı takdirde varlığı siler `RowVersion` .</span><span class="sxs-lookup"><span data-stu-id="86be3-504">Clicking **Delete** deletes the entity, unless `RowVersion` has been updated.</span></span>

<span data-ttu-id="86be3-505">Veri modelinin nasıl devralınacağını öğrenmek için bkz. [Devralma](xref:data/ef-mvc/inheritance) .</span><span class="sxs-lookup"><span data-stu-id="86be3-505">See [Inheritance](xref:data/ef-mvc/inheritance) on how to inherit a data model.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="86be3-506">Ek kaynaklar</span><span class="sxs-lookup"><span data-stu-id="86be3-506">Additional resources</span></span>

* [<span data-ttu-id="86be3-507">EF Core eşzamanlılık belirteçleri</span><span class="sxs-lookup"><span data-stu-id="86be3-507">Concurrency Tokens in EF Core</span></span>](/ef/core/modeling/concurrency)
* [<span data-ttu-id="86be3-508">EF Core eşzamanlılık işle</span><span class="sxs-lookup"><span data-stu-id="86be3-508">Handle concurrency in EF Core</span></span>](/ef/core/saving/concurrency)
* [<span data-ttu-id="86be3-509">Bu öğreticinin YouTube sürümü (eşzamanlılık çakışmalarını Işleme)</span><span class="sxs-lookup"><span data-stu-id="86be3-509">YouTube version of this tutorial(Handling Concurrency Conflicts)</span></span>](https://youtu.be/EosxHTFgYps)
* [<span data-ttu-id="86be3-510">Bu öğreticinin YouTube sürümü (Bölüm 2)</span><span class="sxs-lookup"><span data-stu-id="86be3-510">YouTube version of this tutorial(Part 2)</span></span>](https://www.youtube.com/watch?v=kcxERLnaGO0)
* [<span data-ttu-id="86be3-511">Bu öğreticinin YouTube sürümü (Bölüm 3)</span><span class="sxs-lookup"><span data-stu-id="86be3-511">YouTube version of this tutorial(Part 3)</span></span>](https://www.youtube.com/watch?v=d4RbpfvELRs)

> [!div class="step-by-step"]
> [<span data-ttu-id="86be3-512">Önceki</span><span class="sxs-lookup"><span data-stu-id="86be3-512">Previous</span></span>](xref:data/ef-rp/update-related-data)
