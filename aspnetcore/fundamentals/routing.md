...
Bu belgenin karşıdan yükleme örnekleri belirli `Startup` bir sınıf tarafından etkinleştirilir. Belirli bir örneği çalıştırmak için, *program.cs* öğesini istenen `Startup` sınıfı çağırmak üzere değiştirin.

## <a name="routing-basics"></a>Yönlendirme temelleri

Tüm ASP.NET Core şablonları oluşturulan koda yönlendirmeyi içerir. Yönlendirme, içindeki `Startup.Configure` [Ara yazılım](xref:fundamentals/middleware/index) ardışık düzenine kaydedilir.

Aşağıdaki kod, yönlendirmenin temel bir örneğini göstermektedir:

[!code-csharp[](routing/samples/3.x/RoutingSample/Startup.cs?name=snippet&highlight=8,10)]

Yönlendirme, ve <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseRouting*> <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*>tarafından kaydedilen bir dizi ara yazılımı kullanır:
...
* Kök URL `/`'ye `GET` bir http isteği gönderildiğinde:
...
Yöntemi `MapGet` , bir **uç noktayı**tanımlamak için kullanılır. Uç nokta şöyle olabilir:

* URL ve HTTP yöntemiyle eşleştirerek seçilir.
* , Temsilcisi çalıştırılarak yürütülür.

Uygulama tarafından eşleştirilecek ve çalıştırılabilen uç noktalar ' de `UseEndpoints`yapılandırılır. Örneğin <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapGet*> <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapPost*>,, ve [benzer yöntemler](xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions) , istek temsilcilerinizi yönlendirme sistemine bağlanır.
...
* Arkasından bir alfabetik karakter dizisi ile `/hello/` başlayan HERHANGI bir URL yolu.  `:alpha`yalnızca alfabetik karakterlerle eşleşen bir rota kısıtlaması uygular. [Yol kısıtlamaları](#route-constraint-reference) bu belgenin ilerleyen kısımlarında açıklanmıştır.
...
<xref:Microsoft.AspNetCore.Builder.HealthCheckEndpointRouteBuilderExtensions.MapHealthChecks*> Çağrı bir sistem durumu denetim uç noktası ekler. Bu <xref:Microsoft.AspNetCore.Builder.AuthorizationEndpointConventionBuilderExtensions.RequireAuthorization*> çağrıda zincir, bir yetkilendirme ilkesini uç noktaya iliştirir.

Kimlik <xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*> doğrulama <xref:Microsoft.AspNetCore.Builder.AuthorizationAppBuilderExtensions.UseAuthorization*> ve yetkilendirme ara yazılımını çağırarak ve ekler. Bu ara yazılım, ile <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseRouting*> arasında `UseEndpoints` yerleştirilir ve şunları yapabilir:

* Hangi uç noktanın seçili olduğunu görün `UseRouting`.
* Uç noktaya göndermeden önce <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*> bir yetkilendirme ilkesi uygulayın.

<a name="metadata"></a>

### <a name="endpoint-metadata"></a>Uç nokta meta verileri

Yukarıdaki örnekte, iki uç nokta bulunur, ancak yalnızca sistem durumu denetimi uç noktasına bağlı bir yetkilendirme ilkesi vardır. İstek sistem durumu denetimi uç noktasıyla `/healthz`eşleşiyorsa, bir yetkilendirme denetimi gerçekleştirilir. Bu, uç noktalara eklenen ek verilere sahip olduğunu gösterir. Bu ek verilere uç nokta **meta verileri**denir:
...
* Yürütülebilir dosya: öğesine <xref:Microsoft.AspNetCore.Http.Endpoint.RequestDelegate>sahip.
...
* Numaralandırılabilir: bitiş noktaları koleksiyonu <xref:Microsoft.AspNetCore.Routing.EndpointDataSource> [, kimden kaynağından](xref:fundamentals/dependency-injection)alma yoluyla listelenebilir.
...
* Bitiş noktası, çağrılmadan önce `UseRouting` her zaman null olur.
* Bir eşleşme bulunursa, uç nokta ve `UseRouting` <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*>arasında boş değildir.
* Bir `UseEndpoints` eşleşme bulunduğunda ara yazılım **terminaldir** . [Terminal ara yazılımı](#tm) bu belgede daha sonra tanımlanır.
* Yalnızca eşleşme bulunamadığında `UseEndpoints` yürütme sonrasında ara yazılım.

`UseRouting` Ara yazılım, uç noktayı geçerli bağlama eklemek Için [setendpoint](xref:Microsoft.AspNetCore.Http.EndpointHttpContextExtensions.SetEndpoint*) yöntemini kullanır. `UseRouting` Ara yazılımı özel mantık ile değiştirmek ve uç noktaları kullanmanın avantajlarını almaya devam etmek mümkündür. Uç noktalar, ara yazılım gibi alt düzey bir temel 'tür ve yönlendirme uygulamasıyla birlikte aktarılmaz. Çoğu uygulamanın özel mantık ile değiştirme `UseRouting` gereksinimi yoktur.

`UseEndpoints` Ara yazılım, `UseRouting` ara yazılım ile birlikte kullanılmak üzere tasarlanmıştır. Bir uç noktayı yürütmek için çekirdek mantık karmaşık değildir. Uç <xref:Microsoft.AspNetCore.Http.EndpointHttpContextExtensions.GetEndpoint*> noktayı almak için kullanın ve ardından <xref:Microsoft.AspNetCore.Http.Endpoint.RequestDelegate> özelliğini çağırın.
...
* Ara yazılım, yönlendirmenin `UseRouting` çalıştığı verileri değiştirmeden önce çalışabilir.
    * Genellikle, yönlendirme öncesinde görünen ara yazılım, veya <xref:Microsoft.AspNetCore.Builder.RewriteBuilderExtensions.UseRewriter*> <xref:Microsoft.AspNetCore.Builder.HttpMethodOverrideExtensions.UseHttpMethodOverride*> <xref:Microsoft.AspNetCore.Builder.UsePathBaseExtensions.UsePathBase*>gibi, isteğin bazı özelliğini değiştirir.
* Ara yazılım, ve `UseRouting` arasında <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*> çalıştırılabilir ve uç nokta yürütülmeden önce yönlendirmenin sonuçlarını işleyebilir.
    * Ve `UseRouting` `UseEndpoints`arasında çalışan ara yazılımlar:
      * Genellikle uç noktaları anlamak için meta verileri inceler.
      * , Ve `UseAuthorization` `UseCors`tarafından yapıldığı gibi genellikle güvenlik kararları verir.
    * Ara yazılım ve meta verilerin birleşimi, ilkelerin uç nokta başına yapılandırılmasını sağlar.

Yukarıdaki kodda, uç nokta başına ilkeleri destekleyen özel bir ara yazılım örneği gösterilmektedir. Ara yazılım, gizli verilere erişim *denetim günlüğünü* konsola yazar. Ara yazılım, `AuditPolicyAttribute` meta verileri olan bir uç noktayı *denetlemek* üzere yapılandırılabilir. Bu örnek, yalnızca hassas olarak işaretlenen bitiş noktalarının denetlendiği bir *katılım* modelini gösterir. Örneğin, güvenli olarak işaretlenmemiş her şeyi denetlemek için bu mantığı ters olarak tanımlamak mümkündür. Uç nokta meta veri sistemi esnektir. Bu mantık, kullanım örneğine uygun herhangi bir şekilde tasarlanabilir.
...
Denetim ilkesi meta verileri `AuditPolicyAttribute` , denetleyiciler ve SignalR gibi sınıf tabanlı çerçevelerle daha kolay kullanılmak `Attribute` üzere tanımlanır. *Kod yolu*kullanırken:
...
İle `Approach 1:` gösterilen ara yazılım stili, **Terminal ara yazılımı**. Bu, eşleşen bir işlem yaptığı için Terminal ara yazılımı olarak adlandırılır:

* Yukarıdaki örnekteki `Path == "/"` eşleştirme işlemi, ara yazılım ve `Path == "/Movie"` yönlendirme içindir.
...
* Her iki yaklaşım da işleme ardışık düzeninin sonlandırılmasını sağlar:
    * Ara yazılım, komut zinciri çağırmak `next`yerine dönerek işlem hattını sonlandırır.
...
* `UseAuthorization` Ve `UseCors`gibi ara yazılım içeren uç noktalar arabirimi.
    * Veya `UseAuthorization` `UseCors` ile Terminal ara yazılımı kullanmak, yetkilendirme sistemiyle el ile arabirim oluşturma gerektirir.
...
* Üzerine <xref:Microsoft.AspNetCore.Routing.IEndpointRouteBuilder>bir genişletme yöntemi yazın.
* Kullanarak <xref:Microsoft.AspNetCore.Routing.IEndpointRouteBuilder.CreateApplicationBuilder*>iç içe geçmiş bir ara yazılım işlem hattı oluşturun.
...
* <xref:Microsoft.AspNetCore.Builder.IApplicationBuilder.Build*>içinde bir <xref:Microsoft.AspNetCore.Http.RequestDelegate>ara yazılım ardışık düzeni.
* ' `Map` İ çağırın ve yeni ara yazılım ardışık düzenini sağlayın.
* Uzantı yönteminden tarafından `Map` sunulan Oluşturucu nesnesini döndürün.
...
Bir yönlendirme ara yazılımı yürütüldüğünde, geçerli istekten ' `Endpoint` de <xref:Microsoft.AspNetCore.Http.HttpContext> bir [istek özelliğine](xref:fundamentals/request-features) bir ve rota değeri ayarlar:
...
>
> Daha `RouteContext` sonraki bir sürümde tür kullanılmıyor olarak işaretlenir:
>
> * Öğesine `RouteData.Values` `HttpRequest.RouteValues`geçirin.
> * Uç `RouteData.DataTokens` nokta meta verilerinden [ıdatatokensmetadata](xref:Microsoft.AspNetCore.Routing.IDataTokensMetadata) almak için geçiş yapın.
...
Tüm eşleşen uç noktalar, <xref:Microsoft.AspNetCore.Routing.Matching.EndpointSelector> sonuna kadar her aşamada işlenir. Son `EndpointSelector` aşamadır. En iyi eşleşme ile eşleştirmelerle en yüksek öncelikli uç noktayı seçer. En iyi eşleşme ile aynı önceliğe sahip başka eşleşmeler varsa, belirsiz eşleşme özel durumu oluşur.

Yol önceliği, daha yüksek öncelikli olarak verilen **daha belirli** bir yol şablonuna göre hesaplanır. Örneğin, şablonları `/hello` ve `/{message}`şunları göz önünde bulundurun:

* Her ikisi de URL yoluyla `/hello`eşleşir.
* `/hello`daha özeldir ve bu nedenle daha yüksek önceliğe sahiptir.

Genel olarak, rota önceliği, uygulamada kullanılan URL şemaları türleri için en iyi eşleşmeyi seçme konusunda iyi bir iş olur. Belirsizliği <xref:Microsoft.AspNetCore.Routing.RouteEndpoint.Order> önlemek için yalnızca gerekli olduğunda kullanın.

Yönlendirme tarafından sunulan genişletilebilirlik türleri nedeniyle, yönlendirme sisteminin belirsiz yolların önünde işlem yapmak mümkün değildir. Yol şablonları `/{message:alpha}` gibi bir örnek düşünün `/{message:int}`:
...
Örneğin, şablonları `/Products/List` ve `/Products/{id}`öğesini düşünün. URL yolundan `/Products/List`daha iyi bir eşleşme `/Products/{id}` olduğunu `/Products/List` varsaymak mantıklı olacaktır. , Değişmez değer segmentinin `/List` parametre segmentinden `/{id}`daha iyi önceliğe sahip olduğu kabul edildiği için geçerlidir.
...
Endpoint Routing <xref:Microsoft.AspNetCore.Routing.LinkGenerator> API 'yi içerir. `LinkGenerator`, [dı](xref:fundamentals/dependency-injection)tarafından kullanılabilen bir tek hizmettir. API `LinkGenerator` , yürütülen bir istek bağlamı dışında kullanılabilir. [Etiket Yardımcıları](xref:mvc/views/tag-helpers/intro), HTML Yardımcıları ve [eylem sonuçları](xref:mvc/controllers/actions)gibi ' <xref:Microsoft.AspNetCore.Mvc.IUrlHelper>i kullanan [Mvc. ıurlhelper](xref:Microsoft.AspNetCore.Mvc.IUrlHelper) ve senaryoları, bağlantı oluşturma yetenekleri sağlamak için `LinkGenerator` API 'yi dahili olarak kullanır.
...
>
> * Gelen `GetUri*` isteklerin `Host` üstbilgisini doğrulayan bir uygulama yapılandırmasında uzantı yöntemlerini dikkatle kullanın. Gelen isteklerin `Host` üstbilgisi doğrulanmaz, güvenilir olmayan istek girişi bir görünüm veya sayfadaki URI 'ler içinde istemciye geri gönderilebilir. Tüm üretim uygulamalarının, `Host` üst bilgisini bilinen geçerli değerlere karşı doğrulamak için kendi sunucusunu yapılandırmasını öneririz.
>
> * Veya <xref:Microsoft.AspNetCore.Routing.LinkGenerator> `MapWhen`ile `Map` birlikte ara yazılım içinde dikkatli kullanın. `Map*`yürütülen isteğin temel yolunu değiştirir ve bu da bağlantı oluşturma çıktısını etkiler. Tüm <xref:Microsoft.AspNetCore.Routing.LinkGenerator> API 'ler temel yol belirtilmesine izin verir. Bağlantı oluşturma `Map*` etkisini geri almak için boş bir temel yol belirtin.

### <a name="middleware-example"></a>Ara yazılım örneği

Aşağıdaki örnekte, bir ara yazılım, mağaza ürünlerini <xref:Microsoft.AspNetCore.Routing.LinkGenerator> listeleyen bir eylem yöntemine bağlantı oluşturmak için API 'yi kullanır. Ekleme tarafından bir sınıfa ve çağırarak `GenerateLink` bir uygulamadaki herhangi bir sınıf için kullanılabilir olan bağlantı oluşturucuyu kullanma:
...
İçindeki `{}` belirteçler, yol eşleştirildiği takdirde bağlanan rota parametrelerini tanımlar. Yol segmentinde birden fazla yol parametresi tanımlanabilir, ancak yol parametreleri bir sabit değer ile ayrılmalıdır. Örneğin, `{controller=Home}{action=Index}` ve `{controller}` `{action}`arasında değişmez değer olmadığından geçerli bir yol değil.  Rota parametrelerinin bir adı olmalı ve ek öznitelikler belirtilmiş olabilir.
...
Catch-all parametresi, yol ayırıcı `/` karakterleri de dahil olmak üzere bir URL oluşturmak için kullanıldığında uygun karakterlerin bir kaçar. Örneğin, yol değerlerini `foo/{*path}` `{ path = "my/path" }` içeren yol oluşturulur. `foo/my%2Fpath` Atlanan eğik çizgiye göz önünde edin. Yol ayırıcı karakterlerini yuvarlaklaştırmak için `**` rota parametresi önekini kullanın. İle `foo/{**path}` `{ path = "my/path" }` rota oluşturulur `foo/my/path`.

İsteğe bağlı bir dosya uzantısına sahip bir dosya adı yakalamaya deneyen URL desenlerinin ek konuları vardır. Örneğin, şablonu `files/{filename}.{ext?}`göz önünde bulundurun. Hem hem de `filename` `ext` için değerler olduğunda her iki değer de doldurulur. URL 'de yalnızca bir değeri `filename` varsa, sondaki `.` değer isteğe bağlı olduğundan yol eşleşir. Aşağıdaki URL 'Ler bu rota ile eşleşiyor:

* `/files/myFile.txt`
* `/files/myFile`

Yol parametreleri, parametre adından sonra bir eşittir işaretiyle (`=`) ayırarak varsayılan değer belirtilerek belirlenmiş **varsayılan değerlere** sahip olabilir. Örneğin, `{controller=Home}` için `Home` `controller`varsayılan değer olarak tanımlar. Parametresi için URL 'de hiçbir değer yoksa varsayılan değer kullanılır. Yol parametreleri, parametre adının sonuna bir soru işareti (`?`) eklenerek isteğe bağlı olarak yapılır. Örneğin, `id?`. İsteğe bağlı değerler ve varsayılan yol parametreleri arasındaki fark şudur:
...
| `{controller}/{action}/{id?}`            | `/Products/Details/123` | 123 olarak`id` ayarlanan `Products` denetleyiciye ve `Details` eyleme eşlenir. |
...
* İlk sabit değer, sağdan sola `c`. Bu `/abcd` nedenle, doğru bir şekilde aranır `/ab|c|d`ve bulur.
* Right (`d`) öğesine ait her şey artık Route parametresiyle `{d}`eşleştirildi.
* Sonraki değişmez değer, sağdan sola `a`. Bu `/ab|c|d` nedenle, `a` ayrıldığımız yerden başlayarak arama yapılır `/|a|b|c|d`.
* Right (`b`) değeri artık Route parametresiyle `{b}`eşleştirildi.
* Kalan bir metin yok ve kalan yol şablonu yok, bu nedenle bu bir eşleşmedir.

Aynı şablonu `/a{b}c{d}` ve URL yolunu `/aabcd`kullanarak negatif bir durum örneği aşağıda verilmiştir. , `|` Algoritmanın nasıl çalıştığını görselleştirmeye yardımcı olmak için kullanılır. Bu durum, aynı algoritma tarafından açıklanan bir eşleşme değildir:
* İlk sabit değer, sağdan sola `c`. Bu `/aabcd` nedenle, doğru bir şekilde aranır `/aab|c|d`ve bulur.
* Right (`d`) öğesine ait her şey artık Route parametresiyle `{d}`eşleştirildi.
* Sonraki değişmez değer, sağdan sola `a`. Bu `/aab|c|d` nedenle, `a` ayrıldığımız yerden başlayarak arama yapılır `/a|a|b|c|d`.
* Right (`b`) değeri artık Route parametresiyle `{b}`eşleştirildi.
* Bu noktada, kalan metin `a`vardır ancak algoritmanın ayrıştırılacak yol şablonu kalmadı, bu nedenle bu bir eşleşme değildir.
...
Yol kısıtlamaları, gelen URL 'de bir eşleşme meydana geldiğinde ve URL yolu yol değerlerinde simgeleştirilir yürütülür. Yol kısıtlamaları genellikle yol şablonu aracılığıyla ilişkili rota değerini inceler ve değerin kabul edilebilir olup olmadığı konusunda doğru veya yanlış bir karar vermez. Bazı rota kısıtlamaları, isteğin yönlendirilip yönlendirilmeyeceğini göz önünde bulundurmanız için yol değeri dışındaki verileri kullanır. Örneğin, bir isteği <xref:Microsoft.AspNetCore.Routing.Constraints.HttpMethodRouteConstraint> http fiiline bağlı olarak kabul edebilir veya reddedebilir. Kısıtlamalar, yönlendirme isteklerinde ve bağlantı oluşturmada kullanılır.

> [!WARNING]
> Giriş doğrulaması için kısıtlamaları kullanmayın. Giriş doğrulaması için kısıtlamalar kullanılıyorsa, geçersiz giriş, bulunamayan bir `404` Yanıt ile sonuçlanır. Geçersiz giriş, uygun bir `400` hata iletisiyle hatalı bir istek üretmelidir. Yol kısıtlamaları, belirli bir rota için girdileri doğrulamak üzere değil, benzer yolların belirsizliğini ortadan kaldırmak için kullanılır.
...
| `bool` | `{active:bool}` | `true`, `FALSE` | Veya `true` `false`ile eşleşir. Büyük/küçük harf duyarsız |
| `datetime` | `{dob:datetime}` | `2016-12-31`, `2016-12-31 7:32pm` | Sabit kültürün `DateTime` geçerli bir değeriyle eşleşir. Önceki uyarıya bakın. |
| `decimal` | `{price:decimal}` | `49.99`, `-1,000.01` | Sabit kültürün `decimal` geçerli bir değeriyle eşleşir. Önceki uyarıya bakın.|
| `double` | `{weight:double}` | `1.234`, `-1,001.01e8` | Sabit kültürün `double` geçerli bir değeriyle eşleşir. Önceki uyarıya bakın.|
| `float` | `{weight:float}` | `1.234`, `-1,001.01e8` | Sabit kültürün `float` geçerli bir değeriyle eşleşir. Önceki uyarıya bakın.|
...
| `alpha` | `{name:alpha}` | `Rick` | Dize bir veya daha fazla alfabetik karakterden `a` - `z` oluşmalıdır ve büyük/küçük harfe duyarsız olmalıdır. |
...
> [!WARNING]
> URL 'YI doğrulayan ve CLR türüne dönüştürülen yol kısıtlamaları her zaman sabit kültürü kullanır. Örneğin, CLR türüne `int` dönüştürme veya. `DateTime` Bu kısıtlamalar, URL 'nin yerelleştirilebilir olmadığını varsayar. Framework tarafından sunulan yol kısıtlamaları, yol değerlerinde depolanan değerleri değiştirmez. URL 'den Ayrıştırılan tüm rota değerleri dizeler olarak depolanır. Örneğin, `float` kısıtlama yol değerini bir float öğesine dönüştürmeye çalışır, ancak dönüştürülen değer yalnızca bir float öğesine dönüştürülebileceğini doğrulamak için kullanılır.
...
Uygulamasının uygulanması, `MyCustomConstraint` bir `0` rota parametresine uygulanmasını engeller:
...
* Yolun `0` `{id}` segmentinde engeller.
...
Aşağıdaki kod, içeren bir `id` `0` öğenin işlenmesini önlemek için daha iyi bir yaklaşımdır:

[!code-csharp[](routing/samples/3.x/RoutingSample/Controllers/TestController.cs?name=snippet2)]

Yukarıdaki kodda `MyCustomConstraint` yaklaşım üzerinde aşağıdaki avantajlar bulunur:

* Özel bir kısıtlama gerektirmez.
* Yol parametresi içerdiğinde `0`daha açıklayıcı bir hata döndürür.

## <a name="parameter-transformer-reference"></a>Parametre transformatörü başvurusu

Parametre dönüştürücüler:

* Kullanarak <xref:Microsoft.AspNetCore.Routing.LinkGenerator>bağlantı oluştururken yürütün.
...
Örneğin `slugify` , yol `blog\{article:slugify}` düzeninde bir özel parametre transformatörü `Url.Action(new { article = "MyTestArticle" })` oluşturur. `blog\my-test-article`
...
Bir parametre transformatörü bir yol düzeninde kullanmak için, şunu içinde <xref:Microsoft.AspNetCore.Routing.RouteOptions.ConstraintMap> `Startup.ConfigureServices`kullanarak yapılandırın:

[!code-csharp[](routing/samples/3.x/RoutingSample/StartupConstraint2.cs?name=snippet)]

ASP.NET Core Framework, bir uç noktanın çözümlediği URI 'yi dönüştürmek için parametre dönüştürücüler kullanır. Örneğin, parametre `area`dönüştürücüler, `controller` `action`,, ve `page`ile eşleştirmek için kullanılan rota değerlerini dönüştürür.
...
İlk adım, adresin türüyle eşleşen bir [`IEndpointAddressScheme<TAddress>`](xref:Microsoft.AspNetCore.Routing.IEndpointAddressScheme`1) aday uç nokta kümesini çözümlemek için adresi kullanmaktır.
...
URL oluşturma sorunlarını gidermeye `Microsoft.AspNetCore.Routing` yönelik ilk adım, günlük düzeyini olarak `TRACE`ayarlıyor. `LinkGenerator`işleme hakkında birçok ayrıntıyı günlüğe kaydeder ve bu da sorunları gidermek için yararlı olabilir.
...
* Adres olarak *yol değerlerini* (<xref:Microsoft.AspNetCore.Routing.RouteValuesAddress>) kullanma:
    ...
    * , Etiket yardımcıları, HTML `IUrlHelper`yardımcıları, eylem sonuçları vb. tarafından kullanılan uygulamayı sağlar.
...
Yönlendirme geçerli istekten, geçerli isteğin `HttpContext.Request.RouteValues`rota değerlerine erişir. Geçerli istekle ilişkili değerler **çevresel değerler**olarak adlandırılır. Netlik amacı doğrultusunda, belgeler yöntemlere geçirilen yol değerlerini **açık değerler**olarak ifade eder.
...
* DI <xref:Microsoft.AspNetCore.Routing.LinkGenerator> aracılığıyla [DI](xref:fundamentals/dependency-injection)alır.
...
bir eylem yöntemi için mutlak bir yol içeren bir URL oluşturur. URL, belirtilen `action` adı ve `route` değerleri içerir.

Aşağıdaki kod, geçerli istekten ve açık değerlerden çevresel değerler sağlar: `{ page = "./Edit, id = 17, }`:

[!code-csharp[](routing/samples/3.x/RoutingSample/Pages/Index.cshtml.cs?name=snippet)]

Önceki kod, Razor `url` Düzenle `/Edit/17` sayfası aşağıdaki sayfa yönergesini içerdiğinde olarak ayarlanır:

 `@page "{id:int}"`

Düzenleme sayfasında `"{id:int}"` yol şablonu yoksa, `url` olur. `/Edit?id=17`

MVC 'nin davranışı, burada <xref:Microsoft.AspNetCore.Mvc.IUrlHelper> açıklanan kurallara ek olarak bir karmaşıklık katmanı ekler:

* `IUrlHelper`her zaman geçerli istekten çevresel değerler olarak yol değerlerini sağlar.
* [Iurlhelper. Action](xref:Microsoft.AspNetCore.Mvc.UrlHelperExtensions.Action*) , geliştirici tarafından geçersiz `action` kılınmadığı sürece her zaman geçerli ve `controller` yol değerlerini açık değerler olarak kopyalar.
* [Iurlhelper. Page](xref:Microsoft.AspNetCore.Mvc.UrlHelperExtensions.Page*) , geçersiz kılınmadığı `page` sürece geçerli yol değerini açık bir değer olarak kopyalar. <!--by the user-->
* `IUrlHelper.Page`her zaman geçersiz kılınmadığı sürece `handler` açık `null` değerler olarak geçerli yol değerini geçersiz kılar.

MVC kendi kurallarını takip ettiğinden, kullanıcılar genellikle çevresel değerlerinin davranış ayrıntılarına göre görünür. Geçmiş ve uyumluluk nedenleriyle, `action`,, ve `controller` `page` `handler` gibi belirli rota değerlerinin kendi özel durum davranışları vardır.

Tarafından `LinkGenerator.GetPathByAction` sunulan eşdeğer işlevsellik ve `LinkGenerator.GetPathByPage` Bu bozuklukları uyumluluk `IUrlHelper` açısından çoğaltır.
...
Ya `LinkGenerator` `IUrlHelper` da bu dönüşe `null` yapılan çağrılar genellikle yol değeri doğrulaması anlaşılmadığında oluşur. Yolun sorunu çözüp çözmediğini görmek için daha fazla yol değeri belirtip yol değerlerini geçersiz kılma sorunlarını giderin.

Rota değeri geçersiz kılma, uygulamanın URL şeması, soldan sağa doğru oluşturulmuş bir hiyerarşi ile hiyerarşik olduğu varsayımıyla birlikte çalışıyor. Bu uygulamada nasıl çalıştığını daha kolay `{controller}/{action}/{id?}` bir şekilde öğrenmek için temel denetleyici yönlendirme şablonunu göz önünde bulundurun. Bir değer **değişikliği** , sağda görünen tüm rota değerlerini **geçersiz kılar** . Bu, hiyerarşiyle ilgili varsayımını yansıtır. Uygulamanın için `id`bir ortam değeri varsa ve işlem için farklı bir değer belirtiyorsa `controller`:

* `id``{controller}` , öğesinin `{id?}`solunda olduğu için yeniden kullanılamaz.

Bu ilkeyi gösteren bazı örnekler:

* Açık değerler için `id`bir değer içeriyorsa, ortam değeri `id` yok sayılır. Ve `controller` `action` için çevresel değerler kullanılabilir.
* Açık değerler için `action`bir değer içeriyorsa, için `action` herhangi bir ortam değeri yok sayılır. İçin `controller` çevresel değerler kullanılabilir. İçin `action` açık değeri, için `action`çevresel değerden farklıysa, `id` değer kullanılmaz.  İçin `action` açık değeri, için `action`çevresel değer ile aynıysa, `id` değer kullanılabilir.
* Açık değerler için `controller`bir değer içeriyorsa, için `controller` herhangi bir ortam değeri yok sayılır. İçin `controller` açık değeri, için `controller`çevresel değerden farklıysa, `action` ve `id` değerleri kullanılmaz. İçin `controller` açık değeri, için `controller`çevresel değer ile aynıysa, `action` ve `id` değerleri kullanılabilir.

Bu işlem, öznitelik yollarının ve adanmış geleneksel yolların varlığı tarafından daha karmaşıktır. Yönlendirme parametrelerini kullanarak bir hiyerarşiyi `{controller}/{action}/{id?}` belirtmek gibi denetleyici geleneksel yolları. [Özel geleneksel yollar](xref:mvc/controllers/routing#dcr) ve denetleyicilere yönelik [öznitelik yolları](xref:mvc/controllers/routing#ar) ve Razor Pages için:
...
Yolun bir segmentiyle eşleşmeyen değerler sorgu dizesine eklenir. Aşağıdaki tabloda, yol şablonu `{controller}/{action}/{id?}`kullanılırken sonuç gösterilmektedir.
...
Yukarıdaki kodda, yerelleştirme için `culture` Route parametresi kullanılır. Bunun için `culture` parametre her zaman bir çevresel değer olarak kabul edilir. Ancak, gerekli `culture` değerlerin çalışma şekli nedeniyle parametre bir çevresel değer olarak kabul edilmez:

* `"default"` Yol `culture` şablonunda, yol parametresi solunda `controller`bulunur, bu nedenle değişiklikler geçersiz `controller` `culture`olmaz.
* `"blog"` Yol şablonunda, `culture` yol parametresi, gerekli değerlerde görüntülenen, öğesinin `controller`sağında kabul edilir.
...
* Özel `[MinimumAgeAuthorize]` bir öznitelik kullanan [ıauthorizationpolicyprovider örneği](https://github.com/dotnet/AspNetCore/tree/release/3.0/src/Security/samples/CustomPolicyProvider)
...
* Joker karakterle `*.domain.com`,, veya `www.domain.com` `subdomain.domain.com` `www.subdomain.domain.com` herhangi bir bağlantı noktasında eşleşen ana bilgisayar.
* Bağlantı noktası `*:5000`:, bağlantı noktası 5000 ile herhangi bir konak ile eşleşir.
* Konak ve bağlantı noktası `www.domain.com:5000` : `*.domain.com:5000`veya, ana bilgisayar ve bağlantı noktasıyla eşleşir.

Veya `RequireHost` `[Host]`kullanılarak birden çok parametre belirtilebilir. Kısıtlama, parametrelerden herhangi biri için geçerli olan konaklarla eşleşir. Örneğin,, `[Host("domain.com", "*.domain.com")]` ve `domain.com` `subdomain.domain.com`ile `www.domain.com`eşleşir.

Aşağıdaki kod, rotada `RequireHost` belirtilen Konağı gerektirmek için kullanır:

[!code-csharp[](routing/samples/3.x/RoutingSample/StartupRequireHost.cs?name=snippet)]

Aşağıdaki kod, belirtilen konaklardan herhangi birini gerektirmek için denetleyicideki `[Host]` özniteliğini kullanır:
...
Bu, örneğin, çok önemli olduğunda gecikmeyi daraltmak için temel bir yoldur `10ms`.  `Time 1` Raporlar, `UseRouting` ara yazılım içinde harcanan süreyi çıkararak `Time 2` .
...
`MapMyFramework(...)` Bu, uygulayan <xref:Microsoft.AspNetCore.Builder.IEndpointConventionBuilder>bir çağrıdan Sealed somut tür döndürün. **DO** Çoğu Framework `Map...` yöntemi bu kalıbı izler. `IEndpointConventionBuilder` Arabirim:
...
**CONSIDER** Kendinizinkini <xref:Microsoft.AspNetCore.Routing.EndpointDataSource>yazmayı düşünün. `EndpointDataSource`, uç nokta koleksiyonunu bildirmek ve güncelleştirmek için alt düzey temel değer. `EndpointDataSource`, denetleyiciler ve Razor Pages tarafından kullanılan güçlü bir API 'dir.
...
Varsayılan `EndpointDataSource` olarak **kaydetmeyi denemeyin** . Kullanıcıların çatısını kaydetmesini gerektir <xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*>. Yönlendirmenin felseı, varsayılan olarak hiçbir şey dahil değildir ve `UseEndpoints` uç noktaların kaydedileceği yerdir.
...
Bu yönergeye örnek olarak, `UseAuthorization` ara yazılımı göz önünde bulundurun. Yetkilendirme ara yazılımı, bir geri dönüş ilkesi geçirmenize olanak sağlar. <!-- shown where?  (shown here) --> Belirtilmişse, geri dönüş ilkesi her ikisi için de geçerlidir:
...
Bu <xref:Microsoft.AspNetCore.Mvc.MvcOptions.EnableEndpointRouting> seçenek yönlendirmenin ASP.NET Core 2,1 veya önceki bir sürümünün uç nokta tabanlı Logic mi <xref:Microsoft.AspNetCore.Routing.IRouter>yoksa tabanlı mantığını mı kullanması gerektiğini belirler. Uyumluluk sürümü 2,2 veya üzeri bir değere ayarlandığında, varsayılan değer olur `true`. Önceki yönlendirme mantığını kullanmak `false` için değeri olarak ayarlayın:
...
* URL oluşturma, rastgele genişletilebilirliği destekleyen adreslere dayalıdır:
  ...
  * Bağlantı Oluşturucu API 'sinin dı aracılığıyla kullanılamadığı durumlarda URL 'Leri <xref:Microsoft.AspNetCore.Mvc.IUrlHelper> derlemek için yöntemler sunar.
...
Bu yöntemlerin aşırı yüklemesi, `HttpContext`içeren bağımsız değişkenleri kabul eder. Bu yöntemler işlevsel olarak `Url.Action` eşdeğerdir, `Url.Page` ancak ek esneklik ve seçenekler sunar.
...
* Uç nokta yönlendirme sistemi, devralma <xref:Microsoft.AspNetCore.Routing.IRouter>dahil olmak üzere tabanlı genişletilebilirliği desteklemez <xref:Microsoft.AspNetCore.Routing.Route>.
...
* Standart yollarla MVC/Razor Pages tarafından kullanılan bağlantı oluşturma, mevcut olmayan bir denetleyiciye/eyleme veya sayfaya bağlantı kurmaya çalışırken farklı şekilde davranır.
  ...
  Tabanlı `IRouter`yönlendirme ile, mevcut olmasa veya bir `/Blog/ReadPost/17` `BlogController` `ReadPost` eylem yöntemine sahip olmasalar bile sonuç her zaman olur. Beklendiği gibi, ASP.NET Core 2,2 veya sonraki bir sürümde uç nokta `/Blog/ReadPost/17` yönlendirme eylem yöntemi varsa üretir. *Ancak, uç nokta yönlendirme, eylem yoksa boş bir dize oluşturur.* Kavramsal olarak, uç nokta yönlendirme, eylem mevcut değilse uç noktanın var olduğunu varsaymaz.
...
Bu şablon, gibi `/Products/Details/17` bir URL yoluyla eşleşir. `/Products/Details/Apples` Yol kısıtlamaları <xref:Microsoft.AspNetCore.Routing.IRouteConstraint> , yol değerlerini uygulayıp onları doğrulamak için inceler. Bu örnekte, yol değeri `id` bir tamsayıya dönüştürülebilir olmalıdır. Framework tarafından sunulan yol kısıtlamalarının açıklaması için bkz. [route-Constraint-Reference](#route-constraint-reference) .

, `dataTokens`Ve `defaults`için <xref:Microsoft.AspNetCore.Builder.MapRouteRouteBuilderExtensions.MapRoute*> `constraints`değer kabul etmenin ek aşırı yüklemeleri. Bu parametrelerin tipik kullanımı, anonim türdeki özellik adlarının yol parametre adlarıyla eşleşen anonim olarak yazılmış bir nesneyi geçirmektir.
...
Önceki şablon, benzer `/Blog/All-About-Routing/Introduction` bir URL yoluyla eşleşir ve değerleri `{ controller = Blog, action = ReadArticle, article = All-About-Routing/Introduction }`ayıklar. Ve `controller` `action` için varsayılan yol değerleri, şablonda karşılık gelen hiçbir yol parametresi olmasa bile rota tarafından üretilir. Varsayılan değerler yol şablonunda belirtilebilir. Route parametresi, yol parametre adından önce bir çift yıldız işareti (`**`) görünümüne göre *catch-all* olarak tanımlanır. `article` Catch-all yol parametreleri, URL yolunun kalanını yakalar ve boş dizeyle de aynı olabilir.
...
Yol değerleriyle `{ controller = Products, action = List }`, URL `/Products/List` oluşturulur. Yol değerleri, URL yolunu biçimlendirmek için karşılık gelen yol parametrelerinin yerine kullanılır. , `id` İsteğe bağlı bir yol parametresi olduğundan, URL 'si için `id`bir değer olmadan başarıyla oluşturulur.
...
Her iki URL de aşağıdaki yol tanımıyla (`/Home/Index` ve `/`) bırlıkte gidiş dönüş, URL oluşturmak için kullanılan rota değerlerini oluşturur.

> [!NOTE]
> ASP.NET Core MVC kullanan bir uygulama, doğrudan <xref:Microsoft.AspNetCore.Mvc.Routing.UrlHelper> yönlendirmeye çağırmak yerine URL 'ler oluşturmak için kullanılmalıdır.
...
Küme ayraçları (`{ ... }`) içindeki belirteçler, yol eşleştirildiği takdirde bağlanan *Rota parametrelerini* tanımlar. Bir yol segmentinde birden fazla yol parametresi tanımlayabilirsiniz, ancak bunların bir değişmez değer ile ayrılması gerekir. Örneğin, `{controller=Home}{action=Index}` ve `{controller}` `{action}`arasında değişmez değer olmadığından geçerli bir yol değil. Bu rota parametrelerinin bir adı olmalı ve ek öznitelikler belirtilmiş olabilir.
...
Yol ayırıcısı (`/`) karakterleri de dahil olmak üzere bir URL oluşturmak için, catch-all parametresi uygun karakterleri de çıkar. Örneğin, yol değerlerini `foo/{*path}` `{ path = "my/path" }` içeren yol oluşturulur. `foo/my%2Fpath` Atlanan eğik çizgiye göz önünde edin. Yol ayırıcı karakterlerini yuvarlaklaştırmak için `**` rota parametresi önekini kullanın. İle `foo/{**path}` `{ path = "my/path" }` rota oluşturulur `foo/my/path`.

Yol parametreleri, parametre adından sonra bir eşittir işaretiyle (`=`) ayırarak varsayılan değer belirtilerek belirlenmiş *varsayılan değerlere* sahip olabilir. Örneğin, `{controller=Home}` için `Home` `controller`varsayılan değer olarak tanımlar. Parametresi için URL 'de hiçbir değer yoksa varsayılan değer kullanılır. Yol parametreleri, içinde`?` `id?`olduğu gibi parametre adının sonuna bir soru işareti () eklenerek isteğe bağlı olarak yapılır. İsteğe bağlı değerler ve varsayılan yol parametreleri arasındaki fark, varsayılan değere sahip bir yol parametresinin her zaman bir değer&mdash;ürettiğinden, isteğe bağlı bir parametre yalnızca istek URL 'si tarafından bir değer sağlandığı zaman bir değere sahip olur.
...
| `alpha` | `{name:alpha}` | `Rick` | Dize bir veya daha fazla alfabetik karakterden `a` - `z`oluşmalıdır.  Büyük/küçük harf duyarsız. |
| `regex(expression)` | `{ssn:regex(^\\d{{3}}-\\d{{2}}-\\d{{4}}$)}` | `123-45-6789` | Dize, normal ifadeyle eşleşmelidir. Normal ifade tanımlama hakkında ipuçlarına bakın. |
| `required` | `{name:required}` | `Rick` | URL oluşturma sırasında parametre olmayan bir değerin mevcut olduğunu zorlamak için kullanılır. |
...
Normal ifadeler, Yönlendirme ve C# dili tarafından kullanılanlarla benzerlik olan sınırlayıcıları ve belirteçleri kullanır. Normal ifade belirteçlerinin atlanmalıdır. Yönlendirmenin normal ifadesini `^\d{3}-\d{2}-\d{4}$` kullanmak için:
...
Yerleşik yol kısıtlamalarına ek olarak, <xref:Microsoft.AspNetCore.Routing.IRouteConstraint> arabirimi uygulayarak özel yol kısıtlamaları oluşturulabilir. Arabirim, kısıtlama `false` karşılanıp Aksi takdirde `Match`döndüren `true` tek bir yöntemi içerir. <xref:Microsoft.AspNetCore.Routing.IRouteConstraint>

Özel <xref:Microsoft.AspNetCore.Routing.IRouteConstraint>bir kullanmak için yol kısıtlama türü, uygulamanın uygulamanın hizmet kapsayıcısında kayıtlı <xref:Microsoft.AspNetCore.Routing.RouteOptions.ConstraintMap> olması gerekir. <xref:Microsoft.AspNetCore.Routing.RouteOptions.ConstraintMap> , Yol kısıtlama anahtarlarını bu kısıtlamaları doğrulayan <xref:Microsoft.AspNetCore.Routing.IRouteConstraint> uygulamalarla eşleyen bir sözlüktür. Bir uygulama <xref:Microsoft.AspNetCore.Routing.RouteOptions.ConstraintMap> , hizmetlerin bir parçası olarak `Startup.ConfigureServices` ' de güncelleştirilir [. AddRouting](xref:Microsoft.Extensions.DependencyInjection.RoutingServiceCollectionExtensions.AddRouting*) çağrısı veya doğrudan ile <xref:Microsoft.AspNetCore.Routing.RouteOptions> `services.Configure<RouteOptions>`yapılandırma. Örneğin:
...
Parametre dönüştürücüler, bir uç noktanın çözümlediği URI 'yi dönüştürmek için çerçevesi tarafından kullanılır. Örneğin, ASP.NET Core `area`MVC, `controller` `action`,, ve `page`ile eşleşecek şekilde kullanılan rota değerini dönüştürmek için parametre dönüştürücüler kullanır.
...
Önceki <xref:Microsoft.AspNetCore.Routing.VirtualPathData.VirtualPath> örnek sonunda oluşturulan `/package/create/123`. Sözlük, "paket `operation` yolunu `id` izle" şablonunun ve rota değerlerini sağlar `package/{operation}/{id}`. Ayrıntılar için, [yönlendirme ara yazılımı kullanma](#use-routing-middleware) bölümünde veya [örnek uygulamada](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/fundamentals/routing/samples)örnek koda bakın.

<xref:Microsoft.AspNetCore.Routing.VirtualPathContext> Oluşturucunun ikinci parametresi bir *ortam değerleri*koleksiyonudur. Ortam değerleri, bir geliştiricinin bir istek bağlamı içinde belirtmesi gereken değer sayısını sınırlandırdığından kullanım için uygundur. Geçerli isteğin geçerli yol değerleri, bağlantı oluşturma için çevresel değerler olarak kabul edilir. ASP.NET Core `About` MVC uygulamasının öğesinin `HomeController`eyleminde, ortam değeri `Index` &mdash; `Home` kullanılan eyleme bağlamak için denetleyici yol değerini belirtmeniz gerekmez.
...
Yönlendirme, ' nin <xref:Microsoft.AspNetCore.Routing.IRouter> rotalar uygulamalarını kullanır:
...
URL oluşturma benzer bir yinelemeli işlem izler, ancak yol koleksiyonu <xref:Microsoft.AspNetCore.Routing.IRouter.GetVirtualPath*> metoduna çağıran kullanıcı veya çerçeve kodu ile başlar. Her *yol* , null <xref:Microsoft.AspNetCore.Routing.IRouter.GetVirtualPath*> olmayan bir değer <xref:Microsoft.AspNetCore.Routing.VirtualPathData> döndürülene kadar metodu sırayla çağırılır.
...
Yollar birincil olarak, tarafından <xref:Microsoft.AspNetCore.Routing.VirtualPathContext.Values> belirtilen yol değerlerini kullanır <xref:Microsoft.AspNetCore.Routing.VirtualPathContext.AmbientValues> ve bir URL oluşturma ve hangi değerlerin dahil edileceğini belirlemek için. , <xref:Microsoft.AspNetCore.Routing.VirtualPathContext.AmbientValues> Geçerli istekle eşleştirmeden üretilmiş olan rota değerleri kümesidir. Buna karşılık, <xref:Microsoft.AspNetCore.Routing.VirtualPathContext.Values> geçerli işlem IÇIN istenen URL 'nin nasıl oluşturulacağını belirten rota değerlerdir. , <xref:Microsoft.AspNetCore.Routing.VirtualPathContext.HttpContext> Bir yolun geçerli bağlamla ilişkili hizmetleri veya ek verileri alması durumunda sağlanır.
...
Çıkışı <xref:Microsoft.AspNetCore.Routing.IRouter.GetVirtualPath*> bir <xref:Microsoft.AspNetCore.Routing.VirtualPathData>. <xref:Microsoft.AspNetCore.Routing.VirtualPathData>, öğesinin <xref:Microsoft.AspNetCore.Routing.RouteData>bir paraleldir. <xref:Microsoft.AspNetCore.Routing.VirtualPathData>çıkış URL <xref:Microsoft.AspNetCore.Routing.VirtualPathData.VirtualPath> 'si için ve yol tarafından ayarlanması gereken bazı ek özellikler içerir.
...
<xref:Microsoft.AspNetCore.Builder.MapRouteRouteBuilderExtensions.MapRoute*>yol işleyici parametresini kabul etmez. <xref:Microsoft.AspNetCore.Builder.MapRouteRouteBuilderExtensions.MapRoute*>yalnızca tarafından işlenen rotaları ekler <xref:Microsoft.AspNetCore.Routing.RouteBuilder.DefaultHandler*>. Varsayılan işleyici bir `IRouter`' dir ve işleyici isteği işleyemeyebilir. Örneğin, ASP.NET Core MVC genellikle yalnızca kullanılabilir bir denetleyici ve eylemle eşleşen istekleri işleyen varsayılan bir işleyici olarak yapılandırılır. MVC 'de yönlendirme hakkında daha fazla bilgi için bkz <xref:mvc/controllers/routing>..
...
Önceki şablon, benzer `/Blog/All-About-Routing/Introduction` bir URL yoluyla eşleşir ve değerleri `{ controller = Blog, action = ReadArticle, article = All-About-Routing/Introduction }`ayıklar. Ve `controller` `action` için varsayılan yol değerleri, şablonda karşılık gelen hiçbir yol parametresi olmasa bile rota tarafından üretilir. Varsayılan değerler yol şablonunda belirtilebilir. Route parametresi, yol parametre adından önce bir yıldız işareti (`*`) görünümüne göre *catch-all* olarak tanımlanır. `article` Catch-all yol parametreleri, URL yolunun kalanını yakalar ve boş dizeyle de aynı olabilir.
...
Gibi listelenen yöntemlerin <xref:Microsoft.AspNetCore.Routing.RequestDelegateRouteBuilderExtensions.MapGet*>bazıları için gerekir <xref:Microsoft.AspNetCore.Http.RequestDelegate>. <xref:Microsoft.AspNetCore.Http.RequestDelegate> Yol eşleştiğinde *yol işleyicisi* olarak kullanılır. Bu ailedeki diğer yöntemler, yönlendirme işleyicisi olarak kullanılmak üzere bir ara yazılım ardışık düzeni yapılandırmaya olanak tanır. `Map*` Yöntemi gibi bir işleyiciyi kabul etmez, <xref:Microsoft.AspNetCore.Routing.RequestDelegateRouteBuilderExtensions.MapRoute*>öğesini kullanır <xref:Microsoft.AspNetCore.Routing.RouteBuilder.DefaultHandler*>.
