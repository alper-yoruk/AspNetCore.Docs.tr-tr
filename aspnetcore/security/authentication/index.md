...
<span data-ttu-id="ec73f-121">Bazı durumlarda, çağrısı `AddAuthentication` diğer uzantı yöntemleri tarafından otomatik olarak yapılır.</span><span class="sxs-lookup"><span data-stu-id="ec73f-121">In some cases, the call to `AddAuthentication` is automatically made by other extension methods.</span></span> <span data-ttu-id="ec73f-122">Örneğin, [ASP.NET Core :::no-loc(Identity)::: ](xref:security/authentication/identity)kullanırken `AddAuthentication` dahili olarak çağrılır.</span><span class="sxs-lookup"><span data-stu-id="ec73f-122">For example, when using [ASP.NET Core :::no-loc(Identity):::](xref:security/authentication/identity), `AddAuthentication` is called internally.</span></span>
