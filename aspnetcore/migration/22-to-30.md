...
Kestrel yapılandırmasını (Program.cs) tarafından sunulan [Web ana bilgisayar oluşturucusuna](#hostb) geçirin `ConfigureWebHostDefaults` :
...
Uygulama ana bilgisayarı yerine ile el ile oluşturursa `ConfigureWebHost` `ConfigureWebHostDefaults` , `UseKestrel` Web ana bilgisayar Oluşturucu 'da çağırın:
...
SignalRJavaScript istemcisi sürümünden `@aspnet/signalr` olarak değiştirilmiştir `@microsoft/signalr` . Bu değişikliğe tepki vermek için, dosyalar,  `require` deyimler ve ECMAScript deyimleriyle ilgilipackage.jsbaşvuruları değiştirin `import` .
