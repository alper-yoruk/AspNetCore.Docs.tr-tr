...
* **Yeni bir ASP.NET Core Web uygulaması oluştur** iletişim kutusunda, **.net Core** ve **ASP.NET Core 5,0** ' un seçili olduğunu doğrulayın. **API** şablonunu seçin ve **Oluştur**' a tıklayın.
...
Proje şablonu, `WeatherForecast` [Swagger](xref:tutorials/web-api-help-pages-using-swagger)desteğiyle bir API oluşturur.
...
* Varsayılan tarayıcı ve `https://localhost:<port>/https://localhost:5001/swagger/index.html` ' a gider, burada `<port>` rastgele seçilmiş bir bağlantı noktası numarasıdır.

# <a name="visual-studio-code"></a>[Visual Studio Code](#tab/visual-studio-code)

[!INCLUDE[](~/includes/trustCertVSC.md)]

Uygulamayı çalıştırmak için CTRL + F5 tuşlarına basın. Bir tarayıcıda aşağıdaki URL 'ye gidin: [https://localhost:5001/swagger](https://localhost:5001/swagger)

# <a name="visual-studio-for-mac"></a>[Mac için Visual Studio](#tab/visual-studio-mac)

Uygulamayı **başlatmak**  >  için**hata ayıklamayı Başlat** ' ı seçin. Mac için Visual Studio bir tarayıcı başlatır ve ' a gider `https://localhost:<port>` , burada `<port>` rastgele seçilmiş bir bağlantı noktası numarasıdır. HTTP 404 (bulunamadı) hatası döndürüldü. `/swagger`URL 'ye ekleyin (URL 'yi olarak değiştirin `https://localhost:<port>/swagger` ).

---

Swagger sayfası `/swagger/index.html` görüntülenir. **Al**  >  **deneyin**  >  **Çalıştır**' ı seçin. Sayfa şunu görüntüler:

* Dalgalı tahmin API 'sini test etmek için [kıvrımlı](https://curl.haxx.se/) komutu.
* Dalgalı tahmin API 'sini test etmek için URL.
* Yanıt kodu, gövde ve üstbilgiler.
* Medya türleri ve örnek değer ve şema içeren bir açılır liste kutusu.
...
### <a name="update-the-launchurl"></a>LaunchUrl 'Yi güncelleştirme

*Properties\launchSettings.jstarihinde*, ' `launchUrl` den ' `"swagger"` e Güncelleştir `"api/TodoItems"` :

```json
"launchUrl": "api/TodoItems",
```

Swagger kaldırıldığından, yukarıdaki biçimlendirme, başlatılan URL 'YI aşağıdaki bölümlerde eklenen denetleyicinin GET yöntemine değiştirir.
...
*Veritabanı bağlamı* , bir veri modeli için Entity Framework işlevselliği koordine eden ana sınıftır. Bu sınıf sınıfından türeterek oluşturulur <xref:Microsoft.EntityFrameworkCore.DbContext?displayProperty=fullName> .
...
* **Gözatın** sekmesini seçin ve ardından * * Microsoft girin.
Arama kutusunda **Entityframeworkcore. SqlServer** .
<!-- https://github.com/dotnet/AspNetCore.Docs/issues/19782 Delete this line at RTM -->
* 5,0 RC sürümü kullanılabilir durumda olan **ön sürümü dahil et** onay kutusunu seçin. 
...
![NuGet Paket Yöneticisi](first-web-api/_static/5/vsNuGet.png)
...
Aşağıdaki kodla *Startup.cs* güncelleştirin:
...
## <a name="update-the-posttodoitem-create-method"></a>PostTodoItem Create metodunu güncelleştirme
...
* Başarılı olursa bir [HTTP 201 durum kodu](https://developer.mozilla.org/docs/Web/HTTP/Status/201) döndürür. HTTP 201, sunucuda yeni bir kaynak oluşturan HTTP POST yöntemi için standart yanıttır.
...
* İstenen KIMLIKLE eşleşen hiçbir öğe yoksa, yöntem [404 durum](https://developer.mozilla.org/docs/Web/HTTP/Status/404) <xref:Microsoft.AspNetCore.Mvc.ControllerBase.NotFound%2A> hata kodu döndürür.
...
## <a name="overview-21"></a>Genel Bakış 2,1
...
## <a name="prerequisites-21"></a>Önkoşullar 2,1
...
## <a name="create-a-web-project-21"></a>Web projesi oluşturma 2,1
...
### <a name="test-the-api-21"></a>API 2,1 'YI test etme
...
## <a name="add-a-model-class-21"></a>Model sınıfı ekleme 2,1
...
## <a name="add-a-database-context-21"></a>Veritabanı bağlamı ekleme 2,1
...
## <a name="register-the-database-context-21"></a>Veritabanı bağlamını kaydetme 2,1
...
## <a name="add-a-controller-21"></a>Denetleyici ekleme 2,1
...
## <a name="add-get-methods-21"></a>Get yöntemleri ekleme 2,1
...
## <a name="routing-and-url-paths-21"></a>Yönlendirme ve URL yolları 2,1
...
## <a name="return-values-21"></a>Dönüş değerleri 2,1
...
## <a name="test-the-gettodoitems-method-21"></a>GetTodoItems yöntemini test etme 2,1
...
## <a name="add-a-create-method-21"></a>Create yöntemi ekleme 2,1
...
### <a name="test-the-posttodoitem-method-21"></a>PostTodoItem yöntemini test etme 2,1
...
### <a name="test-the-location-header-uri-21"></a>Konum üst bilgisi URI 2,1 ' i test edin
...
## <a name="add-a-puttodoitem-method-21"></a>PutTodoItem yöntemi ekleme 2,1
...
### <a name="test-the-puttodoitem-method-21"></a>PutTodoItem yöntemini test etme 2,1
...
## <a name="add-a-deletetodoitem-method-21"></a>DeleteTodoItem yöntemi ekleme 2,1
...
### <a name="test-the-deletetodoitem-method-21"></a>DeleteTodoItem yöntemini test etme 2,1
...
## <a name="call-the-web-api-with-javascript-21"></a>JavaScript 2,1 ile Web API 'sini çağırma
...
### <a name="get-a-list-of-to-do-items-21"></a>Yapılacaklar öğelerinin bir listesini alın 2,1
...
### <a name="add-a-to-do-item-21"></a>Yapılacak bir öğe ekleyin 2,1
...
### <a name="update-a-to-do-item-21"></a>Yapılacak bir öğeyi güncelleştirme 2,1
...
### <a name="delete-a-to-do-item-21"></a>Yapılacak bir öğeyi silme 2,1
...
## <a name="add-authentication-support-to-a-web-api-21"></a>Web API 2,1 kimlik doğrulama desteği ekleme

[!INCLUDE[](~/includes/IdentityServer4.md)]

## <a name="additional-resources-21"></a>Ek kaynaklar 2,1
